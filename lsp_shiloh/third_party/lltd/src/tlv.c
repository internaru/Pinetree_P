...
{
    ASSERT(g_osl && IF_NUM > 0);
    int rif = RIF;
    ASSERT(rif >= 0 && rif < IF_NUM);
    qos_info_t *qosInfo = g_qos + rif;
    ASSERT(qosInfo != NULL);
    ...
    IF_TRACED(TRC_TLVINFO)
        DBG_VERBOSE("write-lg_icon: tlvnum:%d  isHello:%s  isLarge:%s  offset:" FMT_SIZET "\n",
               number, isHello==TRUE?"true":"false", isLarge==TRUE?"true":"false", offset);
    END_TRACE

    if (icon->fd_icon < 0)
    {
        DBG_ERR("write-lg_icon: No icon FD, nothing written\n");
...
#ifdef __MARVELL__
    pthread_mutex_lock(&g_osl->icon_info.det_lock);
#endif // __MARVELL__
    ...
    if (curOffset != (off_t)offset)
    {
        ...
        icon->fd_icon = -1;
#else // __MARVELL__
        icon->fd_icon = open(g_jumbo_icon_path, O_RDONLY);
        ASSERT(icon->fd_icon < 0);
#endif // __MARVELL__
        return 0;
    }
    ...
    IF_TRACED(TRC_TLVINFO)
        DBG_VERBOSE("writing jumbo-tlv of %d bytes, with %s remaining\n",
    ...
    if (remaining <= 0)
    {
        ...
        qosInfo->short_reorder_buffer = htons((uint16_t)num_read);
        memcpy(buf, &qosInfo->short_reorder_buffer, 2);
    } else {
        /* found at least one more byte... set the more-flag */
        /* replace: *(uint16_t*)buf = htons((uint16_t)(0x8000) | (uint16_t)num_read); with: */
        qosInfo->short_reorder_buffer = htons((uint16_t)(0x8000) | (uint16_t)num_read);
        memcpy(buf, &qosInfo->short_reorder_buffer, 2);
    }
    IF_TRACED(TRC_TLVINFO)
        DBG_VERBOSE("txbuf: %02X, %02X, %02X, %02X\n",buf[0],buf[1],buf[2],buf[3]);
    END_TRACE
#ifdef __MARVELL__
    pthread_mutex_unlock(&g_osl->icon_info.det_lock);
#else // __MARVELL__
    close(icon->fd_icon);
    icon->fd_icon = -1;
#endif // __MARVELL__
...
write_icon_file_t(int number, void *data, uint8_t *buf, int bytes_free, bool_t isHello, bool_t isLarge, size_t offset)
{
    ...
    IF_TRACED(TRC_TLVINFO)
        DBG_VERBOSE("write-icon-file: tlvnum:%d  isHello:%s  isLarge:%s  offset:" FMT_SIZET "\n",
               number, isHello==TRUE?"true":"false", isLarge==TRUE?"true":"false", offset);
    END_TRACE

    if (icon->fd_icon < 0)
    {
        DBG_ERR("write-icon-file: No icon FD, nothing written\n");
    ...
    if (isHello==TRUE)
    {
        ...
        {
            IF_TRACED(TRC_TLVINFO)
                DBG_ERR("write_icon_file: not enuff room - bytes_free=%d, and icon_sz=" FMT_SIZET "\n",
                       bytes_free, icon->sz_iconfile);
                DBG_ERR("writing small-tlv of 0 bytes to notify of large-tlv value\n");
            END_TRACE
...
#ifdef __MARVELL__
    pthread_mutex_lock(&g_osl->icon_info.lock);
#endif // __MARVELL__
    ...
    if (curOffset != (off_t)offset)
    {
        ...
        icon->fd_icon = -1;
#else // __MARVELL__
        icon->fd_icon = open(g_icon_path, O_RDONLY);
        ASSERT(icon->fd_icon < 0);
#endif // __MARVELL__
        return 0;
    }
    ...
    if (isHello==TRUE)
    {
        /* write header in Hello's small-TLV format */
        IF_TRACED(TRC_TLVINFO)
            DBG_VERBOSE("writing small-tlv of %d bytes\n",num_read+2);
        END_TRACE
        *buf++ = (uint8_t)number;
        *buf-- = (uint8_t)num_read;
        ...
        IF_TRACED(TRC_TLVINFO)
            DBG_VERBOSE("writing large-tlv of %d bytes, with %s remaining\n",
                    num_read+2, remaining<=0?"none":"some");
        END_TRACE
    ...
    END_TRACE
#ifdef __MARVELL__
    pthread_mutex_unlock(&g_osl->icon_info.lock);
#else // __MARVELL__
    close(icon->fd_icon);
    icon->fd_icon = -1;
#endif // __MARVELL__
...
write_etheraddr_t(int number, void *data, uint8_t *buf, int bytes_free, bool_t isHello, bool_t isLarge, size_t offset)
{
    ...
    if (isHello)
    {
        ...
        *buf = (uint8_t)number;
        ...
        qosInfo->short_reorder_buffer = htons(sizeof(etheraddr_t));
        memcpy(buf, &qosInfo->short_reorder_buffer, 2);
    }
...
write_uint8_t(int number, void *data, uint8_t *buf, int bytes_free, bool_t isHello, bool_t isLarge, size_t offset)
{
    ...
    if (isHello)
    {
        ...
        *buf = (uint8_t)number;
        ...
        /* write in QueryLargeTlvResp LTLV format */
        /* replace: *(uint16_t*)buf = htons(sizeof(etheraddr_t)); with: */
        qosInfo->short_reorder_buffer = htons(sizeof(uint8_t));
        memcpy(buf, &qosInfo->short_reorder_buffer, 2);
    }
    *(buf+2) = *(uint8_t*)data;
...
write_uint16_t(int number, void *data, uint8_t *buf, int bytes_free, bool_t isHello, bool_t isLarge, size_t offset)
{
    ...
    if (isHello)
    {
        ...
        *buf = (uint8_t)number;
        ...
        qosInfo->short_reorder_buffer = htons(sizeof(uint16_t));
        memcpy(buf, &qosInfo->short_reorder_buffer, 2);
    }
...
write_uint32_t(int number, void *data, uint8_t *buf, int bytes_free, bool_t isHello, bool_t isLarge, size_t offset)
{
    ...
    if (isHello)
    {
        ...
        *buf = (uint8_t)number;
        ...
        qosInfo->short_reorder_buffer = htons(sizeof(uint32_t));
        memcpy(buf, &qosInfo->short_reorder_buffer, 2);
    }
...
write_uint64_t(int number, void *data, uint8_t *buf, int bytes_free, bool_t isHello, bool_t isLarge, size_t offset)
{
    ...
    if (isHello)
    {
        ...
        *buf = (uint8_t)number;
        ...
        qosInfo->short_reorder_buffer = htons(sizeof(uint64_t));
        memcpy(buf, &qosInfo->short_reorder_buffer, 2);
    }
...
write_ipv6addr_t(int number, void *data, uint8_t *buf, int bytes_free, bool_t isHello, bool_t isLarge, size_t offset)
{
    ...
    if (isHello)
    {
        ...
        *buf = (uint8_t)number;
        ...
        qosInfo->short_reorder_buffer = htons(sizeof(ipv6addr_t));
        memcpy(buf, &qosInfo->short_reorder_buffer, 2);
    }
...
write_ucs2char_t(int number, void *data, uint8_t *buf, int bytes_free, bool_t isHello, bool_t isLarge, size_t offset)
{
    ...
    if (isHello)
    {
        ...
        *buf = (uint8_t)number;
        ...
        qosInfo->short_reorder_buffer = htons(sizeof(bytes_needed));
        memcpy(buf, &qosInfo->short_reorder_buffer, 2);
    }
...
write_assns_t(int number, void *data, uint8_t *buf, int bytes_free, bool_t isHello, bool_t isLarge, size_t offset)
{
    ...
    IF_TRACED(TRC_TLVINFO)
        DBG_VERBOSE("write-assns-list: tlvnum:%d  isHello:%s  isLarge:%s  offset:" FMT_SIZET "\n",
               number, isHello==TRUE?"true":"false", isLarge==TRUE?"true":"false", offset);
    END_TRACE
    ...
    IF_TRACED(TRC_TLVINFO)
        DBG_VERBOSE("writing assn-list as large-tlv of %d bytes\n", bytes_used);
    END_TRACE
...
write_comptbl_t(int number, void *data, uint8_t *buf, int bytes_free, bool_t isHello, bool_t isLarge, size_t offset)
{
    ...
    IF_TRACED(TRC_TLVINFO)
        DBG_VERBOSE("write-component-table: tlvnum:%d  isHello:%s  isLarge:%s  offset:" FMT_SIZET "\n",
               number, isHello==TRUE?"true":"false", isLarge==TRUE?"true":"false", offset);
    END_TRACE
    ...
    *buf++ = (uint8_t)0;
    ...
    if (Bridge_Component != 0xFF)
    {
        ...
        *buf++ = ctbl->bridge_behavior;             // value
        IF_TRACED(TRC_TLVINFO)
            DBG_VERBOSE("w-c-t: version:%d  bridge behavior: %d\n",ctbl->version,ctbl->bridge_behavior);
        END_TRACE
    }
    ...
    /* for each radio band, add a component */
    {
        ...
        for (i=0;i<ctbl->radio_cnt;i++,rb++)
        {
            ...
            *buf++ = rb->mode;
            ...
            IF_TRACED(TRC_TLVINFO)
                DBG_VERBOSE("w-c-t: radio band: %d  MOR: %d  PHY: %d  Mode: %d  BSSID: " ETHERADDR_FMT "\n",
    ...
    if (Switch_Component != 0xFFFFFFFF)
    {
        ...
        *buf++ = 4;                                 // length
        ...
        IF_TRACED(TRC_TLVINFO)
            DBG_VERBOSE("w-c-t: switch link speed: %d\n",ntohl(ctbl->link_speed));
        END_TRACE
    }

    /* Now, once the total length is known, put it in the reserved location at the front */
    bytes_used = buf - length;
    qosInfo->short_reorder_buffer = htons(bytes_used - 2);
    memcpy(length, &qosInfo->short_reorder_buffer, 2);
    return bytes_used;
}
...
tlv_write_tlv(tlv_desc_t *tlv, uint8_t *buf, int bytes_free, bool_t isHello, size_t LtlvOffset)
{
    ...
    uint8_t *infobase = (uint8_t*)(&g_tlv_info);
    int    nb = 0;
    int    getfn_returned = -1;

    IF_TRACED(TRC_TLVINFO)
        DBG_VERBOSE("tlv_write_tlv: Building TLV # %d (%d bytes available in send-buffer\n",
    ...
    switch (tlv->access)
    {
      case  Access_unset:
        if (tlv->get != NULL)
        {
            ...
            IF_TRACED(TRC_TLVINFO)
                DBG_VERBOSE("tlv_write_tlv: initializing 'unset' TLV %d to 'static'\n", tlv->number);
            END_TRACE
        } else {
      ...
      case  Access_dynamic:
        ...
        {   /* call the tlv_get_fn for this tlv */
            ...
            IF_TRACED(TRC_TLVINFO)
                DBG_VERBOSE("tlv_write_tlv: getting value for TLV %d returned \'%s\'\n",
