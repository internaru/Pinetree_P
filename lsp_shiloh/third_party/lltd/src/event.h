...
extern event_t *event_add(int ifndx, struct timeval *firetime, event_fn_t function, void *state);
...
extern void event_add_io(int if_ndx, int fd, event_io_fn_t function, void *state);
extern void event_remove_io(int fd);

/* Capture the current thread, and run event and IO handlers forever.
 * Does not return. */
extern void *event_iotask(void *rif);
