...
wlan_prog_fw_w_helper(IN pmlan_adapter pmadapter, IN pmlan_fw_image pmfw)
{
    ...
    do {
        ...
        if (firmware)
            MOAL_MEMCPY(pmadapter, &fwdata->fw_header, &firmware[TotalBytes],
                   sizeof(FWHeader));
        else
        ...
        if (firmware)
            MOAL_MEMCPY(pmadapter, fwdata->data, &firmware[TotalBytes], DataLength);
        else
            pcb->moal_get_fw_data(pmadapter->pmoal_handle,
        ...
        while (retries) {
            ...
            MOAL_MEMSET(pmadapter, &mbuf, 0, sizeof(mlan_buffer));
            ...
            MOAL_MEMCPY(pmadapter, &SyncFWHeader, RecvBuff, sizeof(FWSyncHeader));
