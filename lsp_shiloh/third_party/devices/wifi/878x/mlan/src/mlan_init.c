...
wlan_init_priv(pmlan_private priv)
{
    ...
    MOAL_MEMSET(pmadapter, priv->curr_addr, 0xff, MLAN_MAC_ADDR_LENGTH);
    ...
    for (i = 0; i < sizeof(priv->wep_key) / sizeof(priv->wep_key[0]); i++)
        MOAL_MEMSET(pmadapter, &priv->wep_key[i], 0, sizeof(mrvl_wep_key_t));
    ...
    MOAL_MEMSET(pmadapter, &priv->curr_bss_params, 0, sizeof(priv->curr_bss_params));
    priv->listen_interval = MLAN_DEFAULT_LISTEN_INTERVAL;

    MOAL_MEMSET(pmadapter, &priv->assoc_rsp_buf, 0, sizeof(priv->assoc_rsp_buf));
    ...
    MOAL_MEMSET(pmadapter, &priv->wpa_ie, 0, sizeof(priv->wpa_ie));
    MOAL_MEMSET(pmadapter, &priv->aes_key, 0, sizeof(priv->aes_key));
    priv->wpa_ie_len = 0;
    ...
    MOAL_MEMSET(pmadapter, &priv->wps, 0, sizeof(priv->wps));
    MOAL_MEMSET(pmadapter, &priv->gen_ie_buf, 0, sizeof(priv->gen_ie_buf));
...
wlan_init_adapter(pmlan_adapter pmadapter)
{
    ...
    if (!pmadapter->init_para.ps_mode) {
        ...
        if (!IS_INIT_PS_MODE(pmadapter->feature_control)) {
    ...
    MOAL_MEMSET(pmadapter, pmadapter->event_body, 0, sizeof(pmadapter->event_body));
    ...
    if (pmadapter->psleep_cfm) {
        ...
        MOAL_MEMSET(pmadapter, &sleep_cfm_buf->ps_cfm_sleep, 0,
    ...
    MOAL_MEMSET(pmadapter, &pmadapter->sleep_params, 0,
           sizeof(pmadapter->sleep_params));
    MOAL_MEMSET(pmadapter, &pmadapter->sleep_period, 0,
    ...
    MOAL_MEMSET(pmadapter, &pmadapter->upld_buf, 0, sizeof(pmadapter->upld_buf));
    pmadapter->upld_len = 0;
    pmadapter->event_cause = 0;
    pmadapter->pmlan_buffer_event = MNULL;
    MOAL_MEMSET(pmadapter, &pmadapter->region_channel, 0,
