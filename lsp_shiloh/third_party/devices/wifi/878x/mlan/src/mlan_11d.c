...
wlan_11d_region_2_code(pmlan_adapter pmadapter, t_u8 * region, OUT t_u8 * code)
{
    ...
    for (i = 0; i < size; i++) {
        if (!MOAL_MEMCMP(pmadapter, region_code_mapping[i].region,
                              ...
                              parsed_region_chan_11d_t * parsed_region_chan)
{
    ...
    MOAL_MEMSET(pmadapter, domain_info, 0, sizeof(wlan_802_11d_domain_reg_t));

    /* Set country code */
    MOAL_MEMCPY(pmadapter,domain_info->country_code,
                              ...
                              BSSDescriptor_t * pbss_desc)
{
    ...
    if (parsed_region_chan->no_of_chan != 0) {
        ...
        MOAL_MEMCPY(pmadapter, parsed_region_chan,
                         ...
                         IEEEtypes_SubbandSet_t * sub_band_list)
{
    ...
    MOAL_MEMSET(pmadapter, pdomain, 0, sizeof(wlan_802_11d_domain_reg_t));
    MOAL_MEMCPY(pmadapter, pdomain->country_code, country_code, COUNTRY_CODE_LEN);
    pdomain->band = band;
    pdomain->no_of_sub_band = num_sub_band;
    MOAL_MEMCPY(pmadapter, pdomain->sub_band, sub_band_list,
...
#ifdef STA_SUPPORT
    MOAL_MEMSET(pmadapter, &(pmadapter->parsed_region_chan), 0,
           sizeof(parsed_region_chan_11d_t));
    MOAL_MEMSET(pmadapter, &(pmadapter->universal_channel), 0,
           sizeof(region_chan_t));
#endif
    MOAL_MEMSET(pmadapter, &(pmadapter->domain_reg), 0,
                             ...
                             HostCmd_DS_COMMAND * pcmd, t_u16 cmd_action)
{
    ...
    MOAL_MEMCPY(pmadapter, domain->country_code,
    ...
    if (no_of_sub_band) {
        MOAL_MEMCPY(pmadapter, domain->sub_band,
...
wlan_11d_create_dnld_countryinfo(mlan_private * pmpriv, t_u8 band)
{
    ...
    if (wlan_11d_is_enabled(pmpriv)) {
        ...
        MOAL_MEMSET(pmadapter, &parsed_region_chan, 0,
               sizeof(parsed_region_chan_11d_t));
                                ...
                                BSSDescriptor_t * pbss_desc)
{
    ...
    if (wlan_11d_is_enabled(pmpriv)) {
        ...
        MOAL_MEMSET(pmadapter, &region_chan, 0, sizeof(parsed_region_chan_11d_t));
        MOAL_MEMSET(pmadapter, &bssdesc_region_chan, 0,
               sizeof(parsed_region_chan_11d_t));

        MOAL_MEMCPY(pmadapter, &region_chan,
               &pmadapter->parsed_region_chan,
               sizeof(parsed_region_chan_11d_t));
