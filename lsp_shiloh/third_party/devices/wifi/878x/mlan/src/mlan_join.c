...
wlan_cmd_append_generic_ie(mlan_private * priv, t_u8 ** ppbuffer)
{
    ...
    if (priv->gen_ie_buf_len) {
        ...
        MOAL_MEMCPY(priv->adapter, *ppbuffer, &ie_header, sizeof(ie_header));
        ...
        *ppbuffer += sizeof(ie_header);
        ...
        MOAL_MEMCPY(priv->adapter, *ppbuffer, priv->gen_ie_buf,
               priv->gen_ie_buf_len);
                        ...
                        BSSDescriptor_t * pbss_desc)
{
    ...
    MOAL_MEMSET(pmriv->adapter, &tsf_tlv, 0x00, sizeof(MrvlIEtypes_TsfTimestamp_t));

    tsf_tlv.header.type = wlan_cpu_to_le16(TLV_TYPE_TSFTIMESTAMP);
    tsf_tlv.header.len = wlan_cpu_to_le16(2 * sizeof(tsf_val));

    MOAL_MEMCPY(pmriv->adapter, *ppbuffer, &tsf_tlv, sizeof(tsf_tlv.header));
    *ppbuffer += sizeof(tsf_tlv.header);
    ...
    MOAL_MEMCPY(pmriv->adapter, *ppbuffer, &tsf_val, sizeof(tsf_val));
    *ppbuffer += sizeof(tsf_val);

    MOAL_MEMCPY(pmriv->adapter, &tsf_val, pbss_desc->time_stamp, sizeof(tsf_val));
                      ...
                      IN t_u32 rate1_size, IN t_u8 * rate2, IN t_u32 rate2_size)
{
    ...
    MOAL_MEMCPY(pmpriv->adapter, tmp, rate1, rate1_size);
    MOAL_MEMSET(pmpriv->adapter, rate1, 0, rate1_size);
                              ...
                              OUT t_u32 * pout_rates_size)
{
    ...
    MOAL_MEMCPY(pmpriv->adapter, pout_rates, pbss_desc->supported_rates,
                           ...
                           IN BSSDescriptor_t * pnew_bss_desc)
{
    ...
    MOAL_MEMCPY(pmpriv->adapter, &new_tsf_base, pnew_bss_desc->time_stamp,
           sizeof(new_tsf_base));
...
wlan_cmd_append_wapi_ie(mlan_private * priv, t_u8 ** ppBuffer)
{
    ...
    if (priv->wapi_ie_len) {
        ...
        *ppBuffer += sizeof(ie_header);
        ...
        MOAL_MEMCPY(priv->adapter, *ppBuffer, priv->wapi_ie, priv->wapi_ie_len);
                          ...
                          IN HostCmd_DS_COMMAND * cmd, IN t_void * pdata_buf)
{
    ...
    MOAL_MEMCPY(pmadapter, passo->peer_sta_addr,
    ...
    MOAL_MEMCPY(pmadapter, pssid_tlv->ssid, pbss_desc->ssid.ssid,
    ...
    MOAL_MEMCPY(pmadapter, &pphy_tlv->fh_ds.ds_param_set,
    ...
    MOAL_MEMCPY(pmadapter, &pmpriv->curr_bss_params.data_rates, rates, rates_size);
    ...
    if (IS_SUPPORT_MULTI_BANDS(pmadapter)
        ...
        MOAL_MEMSET(pmadapter, pchan_tlv->chan_scan_param, 0x00,
    ...
    if (!pmpriv->wps.session_enable) {
        if ((pmpriv->sec_info.wpa_enabled || pmpriv->sec_info.wpa2_enabled)) {
            ...
            if (prsn_ie_tlv->header.len <= (sizeof(pmpriv->wpa_ie) - 2))
                MOAL_MEMCPY(pmadapter, prsn_ie_tlv->rsn_ie, &pmpriv->wpa_ie[2],
            ...
            if (pbss_desc->pwpa_ie) {
                ...
                if (prsn_ie_tlv->header.len <= (sizeof(pmpriv->wpa_ie))) {
                    MOAL_MEMCPY(pmadapter, prsn_ie_tlv->rsn_ie,
    ...
    MOAL_MEMCPY(pmadapter, &tmp_cap, &pbss_desc->cap_info, sizeof(passo->cap_info));
    ...
    MOAL_MEMCPY(pmadapter, &passo->cap_info, &tmp_cap, sizeof(passo->cap_info));
                          ...
                          IN HostCmd_DS_COMMAND * resp, IN t_void * pioctl_buf)
{
    ...
    MOAL_MEMCPY(pmpriv->adapter, pmpriv->assoc_rsp_buf, &resp->params,
    ...
    //dprintf("wlan_ret_802_11_associate:\n");
    wifi_intf_sta_set_state(WLAN_ASSOCIATED);
    wifi_intf_sta_set_link_status(WIFI_INTF_LINK_EVENT_UP);

    //start the bad wep key timer
    wifi_enable_bad_wepkey_timer();
    
    DPRINTF((DBG_SOFT | DBG_OUTPUT),("wlan_ret_802_11_associate\n"));
    ...
    MOAL_MEMCPY(pmpriv->adapter, &pmpriv->curr_bss_params.bss_descriptor,
    ...
    MOAL_MEMCPY(pmpriv->adapter, (t_u8 *) pevent->event_buf,
                             ...
                             IN HostCmd_DS_COMMAND * cmd, IN t_void * pdata_buf)
{
    ...
    MOAL_MEMSET(pmadapter, padhoc_start->ssid, 0, MLAN_MAX_SSID_LENGTH);

    MOAL_MEMCPY(pmadapter, padhoc_start->ssid,
           ((mlan_802_11_ssid *) pdata_buf)->ssid,
           ((mlan_802_11_ssid *) pdata_buf)->ssid_len);

    PRINTM(MINFO, "ADHOC_S_CMD: SSID = %s\n", padhoc_start->ssid);

    MOAL_MEMSET(pmadapter, pbss_desc->ssid.ssid, 0, MLAN_MAX_SSID_LENGTH);
    MOAL_MEMCPY(pmadapter, pbss_desc->ssid.ssid,
           ((mlan_802_11_ssid *) pdata_buf)->ssid,
           ((mlan_802_11_ssid *) pdata_buf)->ssid_len);
...
#define DS_PARA_IE_LEN  1
    ...
    MOAL_MEMCPY(pmadapter, &pbss_desc->phy_param_set,
...
#define IBSS_PARA_IE_LEN  2
    ...
    MOAL_MEMCPY(pmadapter, &pbss_desc->ss_param_set,
...
#define AD_HOC_CAP_PRIVACY_ON   1
    ...
    MOAL_MEMSET(pmadapter, padhoc_start->DataRate, 0,
    ...
    MOAL_MEMCPY(pmadapter, &pmpriv->curr_bss_params.data_rates,
    ...
    if (pmpriv->sec_info.ewpa_enabled) {
        MOAL_MEMCPY(pmadapter, pmpriv->wpa_ie, wpa_ie, sizeof(wpa_ie));
    ...
    if (pmadapter->adhoc_11n_enabled == MTRUE) {
        ...
        }
        {
            ...
            MOAL_MEMSET(pmadapter, pht_info, 0, sizeof(MrvlIETypes_HTInfo_t));
    ...
    MOAL_MEMCPY(pmadapter, &tmp_cap, &padhoc_start->cap, sizeof(t_u16));
    ...
    tmp_cap = wlan_cpu_to_le16(tmp_cap);
    MOAL_MEMCPY(pmadapter, &padhoc_start->cap, &tmp_cap, sizeof(t_u16));
...
#define USE_G_PROTECTION        0x02
    ...
    MOAL_MEMCPY(pmadapter, &padhoc_join->bss_descriptor.bssid,
           &pbss_desc->mac_address, MLAN_MAC_ADDR_LENGTH);

    MOAL_MEMCPY(pmadapter, &padhoc_join->bss_descriptor.ssid,
           &pbss_desc->ssid.ssid, pbss_desc->ssid.ssid_len);

    MOAL_MEMCPY(pmadapter, &padhoc_join->bss_descriptor.phy_param_set,
           &pbss_desc->phy_param_set, sizeof(IEEEtypes_PhyParamSet_t));

    MOAL_MEMCPY(pmadapter, &padhoc_join->bss_descriptor.ss_param_set,
           &pbss_desc->ss_param_set, sizeof(IEEEtypes_SsParamSet_t));
    padhoc_join->bss_descriptor.ss_param_set.ibss_param_set.atim_window = 0;

    MOAL_MEMCPY(pmadapter, &tmp_cap, &pbss_desc->cap_info,
    ...
    PRINTM(MINFO, "ADHOC_J_CMD: tmp_cap=%4X CAPINFO_MASK=%4lX\n",
           tmp_cap, CAPINFO_MASK);
    MOAL_MEMCPY(pmadapter, &padhoc_join->bss_descriptor.cap, &tmp_cap,
           sizeof(IEEEtypes_CapInfo_t));
    ...
    MOAL_MEMSET(pmadapter, padhoc_join->bss_descriptor.data_rates, 0,
           sizeof(padhoc_join->bss_descriptor.data_rates));
    MOAL_MEMCPY(pmadapter, padhoc_join->bss_descriptor.data_rates,
    ...
    if (ISSUPP_11NENABLED(pmadapter->fw_cap_info)
        && IS_ADHOC_11N(pmadapter->feature_control)
        )
    ...
    MOAL_MEMCPY(pmadapter, &tmp_cap, &padhoc_join->bss_descriptor.cap,
           sizeof(IEEEtypes_CapInfo_t));
    tmp_cap = wlan_cpu_to_le16(tmp_cap);

    MOAL_MEMCPY(pmadapter, &padhoc_join->bss_descriptor.cap,
                       ...
                       IN HostCmd_DS_COMMAND * resp, IN t_void * pioctl_buf)
{
    ...
    if (command == HostCmd_CMD_802_11_AD_HOC_START) {
        ...
        MOAL_MEMCPY(pmpriv->adapter, pbss_desc->mac_address,
               ...
               IN t_void * pioctl_buf, IN BSSDescriptor_t * pbss_desc)
{
    ...
    MOAL_MEMCPY(pmpriv->adapter, &current_bssid,
                ...
                IN mlan_ioctl_req * pioctl_req, IN mlan_802_11_mac_addr * mac)
{
    ...
    if (pmpriv->media_connected == MTRUE) {
        if (pmpriv->bss_mode == MLAN_BSS_MODE_INFRA) {
            if (mac) {
                if (!MOAL_MEMCMP(pmpriv->adapter, mac, zero_mac, sizeof(zero_mac)))
                    MOAL_MEMCPY(pmpriv->adapter, (t_u8 *) & mac_address,
