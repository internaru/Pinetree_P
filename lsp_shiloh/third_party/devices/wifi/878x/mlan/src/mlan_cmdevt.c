                   ...
                   IN t_void * pioctl_buf, IN t_void * pdata_buf)
{
    ...
    MOAL_MEMSET(pmadapter, pcmd_node->cmdbuf->pbuf, 0, MRVDRV_SIZE_OF_CMD_BUFFER);
                  ...
                  IN HostCmd_DS_COMMAND * resp, IN mlan_ioctl_req * pioctl_buf)
{
    ...
    if (pioctl_buf) {
        ...
        MOAL_MEMCPY(pmpriv->adapter, misc->param.hostcmd.cmd, (void *) resp, size);
    }
                  ...
                  IN HostCmd_DS_COMMAND * cmd, IN t_void * pdata_buf)
{
    ...
    MOAL_MEMCPY(pmpriv->adapter, (void *) cmd, pcmd_ptr->cmd, pcmd_ptr->len);
...
wlan_dnld_cmd_to_fw(IN mlan_private * pmpriv, IN cmd_ctrl_node * pcmd_node)
{
    ...
    MOAL_MEMCPY(pmadapter, (t_u8 *) pcmd - MLAN_TYPE_LEN, (t_u8 *) & tmp,
           MLAN_TYPE_LEN);
    pcmd_node->cmdbuf->data_offset -= MLAN_TYPE_LEN;
...
wlan_dnld_sleep_confirm_cmd(mlan_adapter * pmadapter)
{
    ...
    MOAL_MEMCPY(pmadapter, pmbuf, pmadapter->psleep_cfm, sizeof(mlan_buffer));
    MOAL_MEMCPY(pmadapter, pmbuf->pbuf + pmbuf->data_offset,
...
wlan_recv_event(pmlan_private priv, mlan_event_id event_id, t_void * pmevent)
{
    ...
    if (pmevent)
        ...
        MOAL_MEMSET(priv->adapter, &mevent, 0, sizeof(mlan_event));
...
wlan_alloc_cmd_buffer(IN mlan_adapter * pmadapter)
{
    ...
    MOAL_MEMSET(pmadapter, pmadapter->cmd_pool, 0, buf_size);
                 ...
                 IN HostCmd_DS_COMMAND * resp, IN mlan_ioctl_req * pioctl_buf)
{
    ...
    if (pioctl_buf) {
        ...
        MOAL_MEMCPY(pmpriv->adapter, info->param.ver_ext.version_str,
...
wlan_cmd_get_hw_spec(IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND * pcmd)
{
    ...
    MOAL_MEMCPY(pmpriv->adapter, hw_spec->permanent_addr, pmpriv->curr_addr,
                     ...
                     IN HostCmd_DS_COMMAND * resp, IN t_void * pioctl_buf)
{
    ...
    if (pmpriv->curr_addr[0] == 0xff)
        MOAL_MEMMOVE(pmadapter, pmpriv->curr_addr, hw_spec->permanent_addr,
                MLAN_MAC_ADDR_LENGTH);
                              ...
                              IN t_u16 cmd_action, IN t_void * pdata_buf)
{
    ...
    MOAL_MEMCPY(pmpriv->adapter, &radio_ctl, pdata_buf, sizeof(t_u32));
