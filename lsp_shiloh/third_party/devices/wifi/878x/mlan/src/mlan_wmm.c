...
wlan_wmm_allocate_ralist_node(pmlan_adapter pmadapter, t_u8 * ra)
{
    ...
    MOAL_MEMCPY(pmadapter, ra_list->ra, ra, MLAN_MAC_ADDR_LENGTH);
                           ...
                           t_u8 * srcAddr, t_u8 tid, t_u8 up, t_u8 status)
{
    ...
    MOAL_MEMCPY(priv->adapter, pOutBuf, (t_u8 *) "TSPEC:", 6);
    pOutBuf += 6;

    MOAL_MEMCPY(priv->adapter, pOutBuf, (t_u8 *) typeStr, wlan_strlen(typeStr));
    ...
    *pOutBuf++ = status + '0';
    ...
    MOAL_MEMCPY(priv->adapter, pOutBuf, (t_u8 *) "]:TID", 5);
    pOutBuf += 5;
    *pOutBuf++ = tid + '0';

    MOAL_MEMCPY(priv->adapter, pOutBuf, (t_u8 *) ":UP", 3);
    pOutBuf += 3;
    *pOutBuf++ = up + '0';
...
wlan_clean_txrx(pmlan_private priv)
{
    ...
    MOAL_MEMCPY(pmadapter, tos_to_tid, ac_to_tid, sizeof(tos_to_tid));
    for (i = 0; i < MAX_NUM_TID; i++) {
        tos_to_tid_inv[tos_to_tid[i]] = (t_u8) i;
    }
...
wlan_wmm_init(pmlan_adapter pmadapter)
{
    ...
    for (j = 0; j < pmadapter->priv_num; ++j) {
        if ((priv = pmadapter->priv[j])) {
            ...
            MOAL_MEMSET(priv->adapter, priv->rx_seq, 0xff, sizeof(priv->rx_seq));
            wlan_wmm_default_queue_priorities(priv);
        }
    }
...
wlan_wmm_get_ralist_node(pmlan_private priv, t_u8 tid, t_u8 * ra_addr)
{
    ...
    while (ra_list && (ra_list != (raListTbl *)
        ...
        if (!MOAL_MEMCMP(priv->adapter, ra_list->ra, ra_addr, MLAN_MAC_ADDR_LENGTH)) {
            LEAVE();
            return ra_list;
        }
        ra_list = ra_list->pnext;
...
wlan_ralist_update(mlan_private * priv, t_u8 * old_ra, t_u8 * new_ra)
{
    ...
    for (tid = 0; tid < MAX_NUM_TID; ++tid) {
        ...
        if (ra_list) {
            ...
            MOAL_MEMCPY(priv->adapter, ra_list->ra, new_ra, MLAN_MAC_ADDR_LENGTH);
        }
    }
...
wlan_wmm_add_buf_txqueue(pmlan_adapter pmadapter, pmlan_buffer pmbuf)
{
    ...
    if (!queuing_ra_based(priv)) {
        ...
        MOAL_MEMCPY(pmadapter, ra, pmbuf->pbuf + pmbuf->data_offset,
               MLAN_MAC_ADDR_LENGTH);
        /** put multicast/broadcast packet in the same ralist */
        if (ra[0] & 0x01)
            MOAL_MEMSET(pmadapter, ra, 0xff, sizeof(ra));
#if defined(UAP_SUPPORT)
                                 ...
                                 IEEEtypes_HTCap_t * pHTCap)
{
    ...
    if ((priv->wmm_required
        ...
        MOAL_MEMCPY(priv->adapter, pwmm_tlv->wmm_ie, &wmm_info_ie[2],
               pwmm_tlv->header.len);
                       ...
                       OUT HostCmd_DS_COMMAND * cmd, IN t_void * pdata_buf)
{
    ...
    MOAL_MEMCPY(pmpriv->adapter, (t_void *) pcmd_ts_status, (t_void *) pts_status,
