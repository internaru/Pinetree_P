                       ...
                       IN t_u16 cmd_action, IN pmlan_ioctl_req pioctl_buf)
{
    ...
    if (MOAL_MEMCMP
        ...
        MOAL_MEMCPY(pmpriv->adapter, tlv_mac->mac, &bss->param.bss_config.mac_addr,
               MLAN_MAC_ADDR_LENGTH);
    ...
    if (bss->param.bss_config.ssid.ssid_len) {
        ...
        MOAL_MEMCPY(pmpriv->adapter, tlv_ssid->ssid, bss->param.bss_config.ssid.ssid,
    ...
    if ((bss->param.bss_config.filter.filter_mode <= MAC_FILTER_MODE_BLOCK_MAC)
        ...
        MOAL_MEMCPY(pmpriv->adapter, tlv_mac_filter->mac_address,
    ...
    if ((bss->param.bss_config.protocol & PROTOCOL_WPA) ||
        ...
        if (bss->param.bss_config.wpa_cfg.length) {
            ...
            MOAL_MEMCPY(pmpriv->adapter, tlv_passphrase->passphrase,
        ...
        if ((bss->param.bss_config.wep_cfg.key0.length) &&
            ...
            MOAL_MEMCPY(pmpriv->adapter, tlv_wep_key->key,
    ...
    if ((bss->param.bss_config.ht_cap_info)
        && (IS_UAP_11N(pmpriv->adapter->feature_control))
        ) {
        ...
        MOAL_MEMCPY(pmpriv->adapter, tlv_htcap->ht_cap.supported_mcs_set,
    ...
    if (bss->param.bss_config.wmm_para.qos_info == 0x80 ||
        ...
        MOAL_MEMCPY(pmpriv->adapter, tlv_wmm_parameter->wmm_para.ouitype,
                           ...
                           IN pmlan_ioctl_req pioctl_buf, IN t_void * pdata_buf)
{
    ...
    if (pioctl_buf->req_id == MLAN_IOCTL_BSS) {
        ...
        if (bss->sub_command == MLAN_OID_BSS_MAC_ADDR) {
            ...
            mac_tlv->header.len = wlan_cpu_to_le16(MLAN_MAC_ADDR_LENGTH);
            if (cmd_action == HostCmd_ACT_GEN_SET)
                MOAL_MEMCPY(pmpriv->adapter, mac_tlv->mac, &bss->param.mac_addr,
                       MLAN_MAC_ADDR_LENGTH);
        ...
        if ((misc->sub_command == MLAN_OID_MISC_GEN_IE) &&
            ...
            && (IS_WAPI(pmpriv->adapter->feature_control))
            ) {
            cmd->size =
                wlan_cpu_to_le16(sizeof(HostCmd_DS_SYS_CONFIG) - 1 + S_DS_GEN +
                                 sizeof(MrvlIEtypesHeader_t) +
            ...
            if (cmd_action == HostCmd_ACT_GEN_SET)
                MOAL_MEMCPY(pmpriv->adapter, ie, misc->param.gen_ie.ie_data,
                       misc->param.gen_ie.len);
        }
                           ...
                           IN mlan_ioctl_req * pioctl_buf)
{
    ...
    while (tlv_buf_left >= sizeof(MrvlIEtypesHeader_t)) {
        ...
        case TLV_TYPE_UAP_MAC_ADDRESS:
            tlv_mac = (MrvlIEtypes_MacAddr_t *) tlv;
            MOAL_MEMCPY(pmpriv->adapter, &bss->param.bss_config.mac_addr,
                   tlv_mac->mac, MLAN_MAC_ADDR_LENGTH);
            break;
        case TLV_TYPE_SSID:
            tlv_ssid = (MrvlIEtypes_SsIdParamSet_t *) tlv;
            bss->param.bss_config.ssid.ssid_len = tlv_len;
            MOAL_MEMCPY(pmpriv->adapter, bss->param.bss_config.ssid.ssid,
        ...
        case TLV_TYPE_RATES:
            tlv_rates = (MrvlIEtypes_RatesParamSet_t *) tlv;
            MOAL_MEMCPY(pmpriv->adapter, bss->param.bss_config.rates,
        ...
        case TLV_TYPE_UAP_WPA_PASSPHRASE:
            ...
            MOAL_MEMCPY(pmpriv->adapter, bss->param.bss_config.wpa_cfg.passphrase,
        ...
        case TLV_TYPE_UAP_PSK:
            ...
            MOAL_MEMCPY(pmpriv->adapter, bss->param.bss_config.psk, tlv_psk->psk,
                   tlv_len);
            break;
#endif /* WIFI_DIRECT_SUPPORT */
        ...
        case TLV_TYPE_UAP_WEP_KEY:
            ...
            if (pkey) {
                ...
                MOAL_MEMCPY(pmpriv->adapter, pkey->key, tlv_wep_key->key,
                       pkey->length);
            }
            break;
        ...
        case HT_CAPABILITY:
            ...
            MOAL_MEMCPY(pmpriv->adapter, bss->param.bss_config.supported_mcs_set,
    ...
    if (pioctl_buf) {
        if (pioctl_buf->req_id == MLAN_IOCTL_BSS) {
            ...
            if (bss->sub_command == MLAN_OID_BSS_MAC_ADDR) {
                if (TLV_TYPE_UAP_MAC_ADDRESS ==
                    ...
                    MOAL_MEMCPY(pmpriv->adapter, &bss->param.mac_addr, tlv->mac,
                           MLAN_MAC_ADDR_LENGTH);
                }
            } else if ((bss->sub_command == MLAN_OID_UAP_BSS_CONFIG) &&
        ...
        if (pioctl_buf->req_id == MLAN_IOCTL_MISC_CFG) {
            ...
            if ((pioctl_buf->action == MLAN_ACT_GET) &&
                ...
                MOAL_MEMCPY(pmpriv->adapter, &misc->param.cust_ie, cust_ie,
        ...
        case TLV_TYPE_UAP_MAC_ADDRESS:
            MOAL_MEMCPY(pmpriv->adapter, pmpriv->curr_addr, tlv->mac,
                   MLAN_MAC_ADDR_LENGTH);
            break;
        default:
            break;
                      ...
                      IN mlan_ioctl_req * pioctl_buf)
{
    ...
    if (psnmp_mib->query_type == HostCmd_ACT_GEN_GET) {
        ...
        while (tlv_buf_left >= sizeof(MrvlIEtypes_snmp_oid_t)) {
            ...
            MOAL_MEMCPY(pmadapter, &data, psnmp_oid, sizeof(t_u32));
            switch (tlv_type) {
                        ...
                        IN HostCmd_DS_COMMAND * cmd, IN t_void * pdata_buf)
{
    ...
    MOAL_MEMCPY(pmpriv->adapter, pcmd_sta_deauth->mac, deauth->mac_addr,
           MLAN_MAC_ADDR_LENGTH);
                          ...
                          IN t_u16 cmd_oid, IN t_void * pdata_buf)
{
    ...
    MOAL_MEMSET(pmpriv->adapter, &pkey_material->key_param_set, 0,
    ...
    if (pkey->is_wapi_key) {
        ...
        if (0 != MOAL_MEMCMP(pmpriv->adapter, pkey->mac_addr, bc_mac, sizeof(bc_mac))) {     /* WAPI 
                                                                                           pairwise 
                                                                                           key: 
                                                                                           unicast 
                                                                                         */
        ...
        MOAL_MEMCPY(pmpriv->adapter, &pkey_material->key_param_set.key[2],
        ...
        MOAL_MEMCPY(pmpriv->adapter, tlv->mac, pkey->mac_addr, MLAN_MAC_ADDR_LENGTH);
        cmd->size =
    ...
    if (pkey->key_len == WPA_AES_KEY_LEN) {
        ...
        if (MOAL_MEMCMP(pmpriv->adapter, pkey->mac_addr, bc_mac, sizeof(bc_mac)))    /* AES 
                                                                                   pairwise 
                                                                                   key: 
                                                                                   unicast 
                                                                                 */
        ...
        if (MOAL_MEMCMP(pmpriv->adapter, pkey->mac_addr, bc_mac, sizeof(bc_mac)))    /* TKIP 
                                                                                   pairwise 
                                                                                   key: 
                                                                                   unicast 
                                                                                 */
    ...
    if (pkey_material->key_param_set.key_type_id) {
        ...
        MOAL_MEMCPY(pmpriv->adapter, pkey_material->key_param_set.key,
               pkey->key_material, pkey->key_len);
        pkey_material->key_param_set.length =
                      ...
                      IN mlan_ioctl_req * pioctl_buf)
{
    ...
    if (pioctl_buf) {
        ...
        for (i = 0; i < info->param.sta_list.sta_count; i++) {
            MOAL_MEMCPY(pmpriv->adapter, info->param.sta_list.info[i].mac_address,
                          ...
                          sta_node * sta_ptr)
{
    ...
    while (tlv_buf_left >= (int) sizeof(MrvlIEtypesHeader_t)) {
        ...
        if (tlv_type == TLV_TYPE_UAP_MGMT_FRAME) {
            ...
            MOAL_MEMCPY(priv->adapter, &frame_control,
    ...
    while (tlv_buf_left >= (int) sizeof(MrvlIEtypesHeader_t)) {
        ...
        if (tlv_type == TLV_TYPE_AP_WAPI_INFO) {
            ...
            DBG_HEXDUMP(MCMD_D, "Fw:multicast_PN", wapi_tlv->multicast_PN,
                        PN_SIZE);
            MOAL_MEMCPY(priv->adapter, (t_u8 *) tx_pn, wapi_tlv->multicast_PN,
                   PN_SIZE);
            for (i = 0; i < 4; i++)
                tx_pn[i] = mlan_ntohl(tx_pn[i]);
            MOAL_MEMCPY(priv->adapter, wapi_tlv->multicast_PN, (t_u8 *) tx_pn,
                   PN_SIZE);
                             ...
                             pmlan_buffer pmbuf)
{
    ...
    MOAL_MEMCPY(priv->adapter, pevent->event_buf,
        ...
        if (tlv_type == TLV_TYPE_UAP_MGMT_FRAME) {
            ...
            if ((mgmt_tlv->frame_control.type == 0) &&
                ...
                assoc_req_ie =
                    (t_u8 *) tlv + sizeof(MrvlIETypes_MgmtFrameSet_t) +
                    assoc_ie_len;
                MOAL_MEMCPY(priv->adapter, pevent->event_buf + pevent->event_len,
                       assoc_req_ie, ie_len);
...
wlan_ops_uap_process_event(IN t_void * priv)
{
    ...
    MOAL_MEMSET(pmadapter, &pevent->event_id, 0, sizeof(pevent->event_id));
    ...
    case EVENT_MICRO_AP_BSS_START:
        ...
        MOAL_MEMCPY(pmadapter, pmpriv->curr_addr, pmadapter->event_body + 2,
               MLAN_MAC_ADDR_LENGTH);
    ...
    case EVENT_MICRO_AP_STA_ASSOC:
        ...
        MOAL_MEMCPY(pmadapter, sta_addr, pmadapter->event_body + 2,
               MLAN_MAC_ADDR_LENGTH);
        ...
        if (pmpriv->is_11n_enabled) {
            ...
            MOAL_MEMSET(pmadapter, sta_ptr->rx_seq, 0xff, sizeof(sta_ptr->rx_seq));
        }
    ...
    case EVENT_MICRO_AP_STA_DEAUTH:
        ...
        MOAL_MEMCPY(pmadapter, (t_u8 *) pevent->event_buf,
