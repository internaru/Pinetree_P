...
mlan_register(IN pmlan_device pmdevice, OUT t_void ** ppmlan_adapter)
{
    ...
    for (i = 0; i < MLAN_MAX_BSS_NUM; i++) {
        ...
        if (pmdevice->bss_attr[i].active == MTRUE) {
            ...
            pmadapter->priv_num++;
            MOAL_MEMSET(pmadapter, pmadapter->priv[i], 0, sizeof(mlan_private));
            ...
            for (j = 0; mlan_ops[j]; j++) {
                if (mlan_ops[j]->bss_role == GET_BSS_ROLE(pmadapter->priv[i])) {
                    MOAL_MEMCPY(pmadapter, &pmadapter->priv[i]->ops, mlan_ops[j],
...
mlan_unregister(IN t_void * pmlan_adapter)
{
    ...
    pmlan_callbacks pcb;
    mlan_status(*m_free) (IN t_void * pmoal_handle, IN t_u8 * pbuf);
    t_s32 i = 0;
    ...
    /* Free mlan_adapter */
    void *tmp = pmadapter->pmoal_handle;
    m_free = pcb->moal_mfree;
    pcb->moal_memset(pmadapter, pmadapter,
                     0, sizeof(mlan_adapter));
    m_free(tmp, (t_u8 *) pmadapter);
    pmadapter = MNULL;
...
mlan_recv(IN t_void * pmlan_adapter, IN pmlan_buffer pmbuf, IN t_u32 port)
{
    ...
    case MLAN_USB_EP_CMD_EVENT:
        ...
        case MLAN_USB_TYPE_EVENT:
            ...
            MOAL_MEMMOVE(pmadapter, &event_cause, pbuf, sizeof(t_u32));
            ...
            if ((len > 0) && (len < MAX_EVENT_SIZE))
                MOAL_MEMMOVE(pmadapter, pmadapter->event_body, pbuf, len);
