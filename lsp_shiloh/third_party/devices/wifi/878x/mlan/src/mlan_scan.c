...
search_oui_in_ie(mlan_adapter * pmadapter, IEBody * ie_body, t_u8 * oui)
{
    ...
    while (count) {
        if (!MOAL_MEMCMP(pmadapter, ie_body->PtkBody, oui, sizeof(ie_body->PtkBody))) {
            LEAVE();
                              ...
                              IN t_u8 filtered_scan)
{
    ...
    int limit_to_chan = wifi_get_scan_channel_restriction();
    ...
    for (region_idx = 0;
        ...
        for (next_chan = 0;
            ...
            if ((limit_to_chan > 0) && ((t_u8)cfp->channel != limit_to_chan))
            {
                // Someone outside is forcing us to limit scans to a specific
                // channel - this isn't the right channel, so skip it.
                continue;
            }
...
wlan_add_wps_probe_request_ie(IN mlan_private * pmpriv, OUT t_u8 ** pptlv_out)
{
    ...
    if (pmpriv->wps.wps_ie.vend_hdr.len) {
        ...
        *pptlv_out += sizeof(MrvlIEtypesHeader_t);
        MOAL_MEMCPY(pmpriv->adapter, *pptlv_out,
                       ...
                       IN ChanScanParamSet_t * pscan_chan_list)
{
    ...
    while (ptmp_chan_list->chan_number) {
        ...
        while (tlv_idx < max_chan_per_scan && ptmp_chan_list->chan_number &&
            ...
            MOAL_MEMCPY(pmadapter, pchan_tlv_out->chan_scan_param + tlv_idx,
                            ...
                            OUT t_u8 * pscan_current_only)
{
    ...
    *pscan_current_only = MFALSE;
    ...
    if (puser_scan_in) {
         ...
         */
        MOAL_MEMCPY(pmadapter, pscan_cfg_out->specific_bssid,
        ...
        for (ssid_idx = 0; ((ssid_idx < NELEMENTS(puser_scan_in->ssid_list))
            ...
            MOAL_MEMCPY(pmadapter, pwildcard_ssid_tlv->ssid,
        ...
        if ((ssid_idx && ssid_filter) ||
            MOAL_MEMCMP(pmadapter, pscan_cfg_out->specific_bssid, &zero_mac,
                   sizeof(zero_mac))) {
    ...
    /* Append rates tlv */
    MOAL_MEMSET(pmadapter, rates, 0, sizeof(rates));
    ...
    MOAL_MEMCPY(pmadapter, prates_tlv->rates, rates, rates_size);
    ...
    if (ISSUPP_11NENABLED(pmpriv->adapter->fw_cap_info)
        ...
        MOAL_MEMSET(pmadapter, pht_cap, 0, sizeof(MrvlIETypes_HTCap_t));
                                ...
                                IN t_u8 ** pbeacon_info, IN t_u32 * bytes_left)
{
    ...
    if (*bytes_left >= sizeof(beacon_size)) {
        ...
        MOAL_MEMCPY(pmadapter, &beacon_size, *pbeacon_info, sizeof(beacon_size));
    ...
    *bytes_left -= beacon_size;
    ...
    MOAL_MEMCPY(pmadapter, pbss_entry->mac_address, pcurrent_ptr,
    ...
    MOAL_MEMCPY(pmadapter, fixed_ie.time_stamp, pcurrent_ptr, 8);
    MOAL_MEMCPY(pmadapter, pbss_entry->time_stamp, pcurrent_ptr, 8);
    pcurrent_ptr += 8;
    bytes_left_for_current_beacon -= 8;

    /* Beacon interval is 2 bytes long */
    MOAL_MEMCPY(pmadapter, &fixed_ie.beacon_interval, pcurrent_ptr, 2);
    pbss_entry->beacon_period = wlan_le16_to_cpu(fixed_ie.beacon_interval);
    pcurrent_ptr += 2;
    bytes_left_for_current_beacon -= 2;

    /* Capability information is 2 bytes long */
    MOAL_MEMCPY(pmadapter, &fixed_ie.capabilities, pcurrent_ptr, 2);
    ...
    MOAL_MEMCPY(pmadapter, &pbss_entry->cap_info, pcap_info,
    ...
    while (bytes_left_for_current_beacon >= 2) {
        ...
        case SSID:
            ...
            MOAL_MEMCPY(pmadapter, pbss_entry->ssid.ssid, (pcurrent_ptr + 2),
                   element_len);
        ...
        case SUPPORTED_RATES:
            ...
            MOAL_MEMCPY(pmadapter, pbss_entry->data_rates, pcurrent_ptr + 2,
                   element_len);
            MOAL_MEMCPY(pmadapter, pbss_entry->supported_rates, pcurrent_ptr + 2,
                   element_len);
        ...
        case FH_PARAM_SET:
            ...
            MOAL_MEMCPY(pmadapter, &pbss_entry->phy_param_set.fh_param_set,
        ...
        case DS_PARAM_SET:
            ...
            MOAL_MEMCPY(pmadapter, &pbss_entry->phy_param_set.ds_param_set,
                   pds_param_set, sizeof(IEEEtypes_DsParamSet_t));
            break;

        case CF_PARAM_SET:
            pcf_param_set = (IEEEtypes_CfParamSet_t *) pcurrent_ptr;
            MOAL_MEMCPY(pmadapter, &pbss_entry->ss_param_set.cf_param_set,
        ...
        case IBSS_PARAM_SET:
            ...
            MOAL_MEMCPY(pmadapter, &pbss_entry->ss_param_set.ibss_param_set,
        ...
        case COUNTRY_INFO:
            ...
            MOAL_MEMCPY(pmadapter, &pbss_entry->country_info,
        ...
        case EXTENDED_SUPPORTED_RATES:
            ...
            if (found_data_rate_ie) {
                ...
                prate += rate_size;
                MOAL_MEMCPY(pmadapter, prate, pcurrent_ptr + 2, bytes_to_copy);
        ...
        case VENDOR_SPECIFIC_221:
            ...
            if (!MOAL_MEMCMP
                ...
                if (total_ie_len == sizeof(IEEEtypes_WmmParameter_t)
                     ...
                     * WMM Parameter IE.
                     */
                    MOAL_MEMCPY(pmadapter, (t_u8 *) & pbss_entry->wmm_ie,
                           pcurrent_ptr, total_ie_len);
                    HEXDUMP("InterpretIE: Resp WMM_IE",
                            (t_u8 *) & pbss_entry->wmm_ie, total_ie_len);
                }
            } else if (!MOAL_MEMCMP(pmadapter, pvendor_ie->vend_hdr.oui,
                               wps_oui, sizeof(wps_oui))) {
                if (pbss_entry->pwps_ie == NULL) {
                    pbss_entry->pwps_ie =
                        (IEEEtypes_VendorSpecific_t *) pcurrent_ptr;
        ...
        case RSN_IE:
            ...
            break;
        case WAPI_IE:
            if (!IS_WAPI(pmadapter->feature_control))
                break;
                            ...
                            IN BSSDescriptor_t * pbss_entry)
{
    ...
    if (pbss_entry->pbeacon_buf) {
        ...
        if (pbss_entry->pwps_ie) {
            pbss_entry->pwps_ie = (IEEEtypes_VendorSpecific_t *)
                (pbss_entry->pbeacon_buf + pbss_entry->wps_offset);
        }
        ...
        pbss_entry->pwps_ie = MNULL;
        pbss_entry->wps_offset = 0;
                                  ...
                                  IN BSSDescriptor_t * pnew_beacon)
{
    ...
    if (pmadapter->pscan_table[beacon_idx].pbeacon_buf) {
        ...
        if (new_bcn_size == old_bcn_size) {
             ...
             */
            MOAL_MEMCPY(pmadapter, pbcn_store,
            ...
            MOAL_MEMCPY(pmadapter, pbcn_store, pnew_beacon->pbeacon_buf,
                   new_bcn_size);
        ...
        if ((pmadapter->pbcn_buf_end + pnew_beacon->beacon_buf_size +
            ...
            if ((ret == MLAN_STATUS_SUCCESS) && (tmp_buf)) {
                ...
                MOAL_MEMCPY(pmadapter, tmp_buf, pmadapter->bcn_buf, bcn_size);
                /* Adjust beacon buffer pointers that are past the current */
                for (adj_idx = 0; adj_idx < num_of_ent; adj_idx++) {
                    bcn_offset =
                        pmadapter->pscan_table[adj_idx].pbeacon_buf -
        ...
        if (pmadapter->pbcn_buf_end + pnew_beacon->beacon_buf_size +
             ...
             */
            MOAL_MEMCPY(pmadapter, pmadapter->pbcn_buf_end, pnew_beacon->pbeacon_buf,
...
wlan_restore_curr_bcn(IN mlan_private * pmpriv)
{
    ...
    if (pmpriv->pcurr_bcn_buf &&
        ...
        MOAL_MEMCPY(pmadapter, pmadapter->pbcn_buf_end, pmpriv->pcurr_bcn_buf,
        ...
        if (pcurr_bss->pwps_ie) {
            pcurr_bss->pwps_ie = (IEEEtypes_VendorSpecific_t *)
                (pcurr_bss->pbeacon_buf + pcurr_bss->wps_offset);
        }
...
wlan_scan_process_results(IN mlan_private * pmpriv)
{
    ...
    if (pmpriv->media_connected == MTRUE) {
        ...
        if (j >= 0) {
            ...
            MOAL_MEMCPY(pmadapter, &pmpriv->curr_bss_params.bss_descriptor,
...
wlan_scan_delete_table_entry(IN mlan_private * pmpriv, IN t_s32 table_idx)
{
    ...
    if (beacon_buf_adj) {
         ...
         *
         * Use MOAL_MEMMOVE since the dest/src memory regions overlap.
         */
        MOAL_MEMMOVE(pmadapter, pbeacon_buf,
    ...
    for (del_idx = table_idx; (del_idx + 1) < pmadapter->num_in_scan_table;
        ...
        MOAL_MEMCPY(pmadapter, pmadapter->pscan_table + del_idx,
    ...
    MOAL_MEMSET(pmadapter, pmadapter->pscan_table + pmadapter->num_in_scan_table - 1,
    ...
    if (pbss_desc->bss_mode == mode) {
        if (pmpriv->sec_info.wep_status == Wlan802_11WEPDisabled
            ...
            return index;
                   ...
                   && pmpriv->sec_info.wpa_enabled
                   /* 
                    * both wpa_enabled and wpa2_enabled can be set
                    *&& !pmpriv->sec_info.wpa2_enabled
                    */
                   && ((pbss_desc->pwpa_ie) &&
                       ((*(pbss_desc->pwpa_ie)).vend_hdr.element_id == WPA_IE))
            ...
            return index;
                   ...
                   /* 
                    * NETGEAR WNHDE111 sets both wpa_enabled and wpa2_enabled
                    *&& !pmpriv->sec_info.wpa_enabled
                    */
                   && pmpriv->sec_info.wpa2_enabled
                   && ((pbss_desc->prsn_ie) &&
...
wlan_flush_scan_table(IN pmlan_adapter pmadapter)
{
    ...
    MOAL_MEMSET(pmadapter, pmadapter->pscan_table, 0,
           (sizeof(BSSDescriptor_t) * MRVDRV_MAX_BSSID_LIST));
    pmadapter->num_in_scan_table = 0;

    MOAL_MEMSET(pmadapter, pmadapter->bcn_buf, 0, pmadapter->bcn_buf_size);
    pmadapter->pbcn_buf_end = pmadapter->bcn_buf;
                   ...
                   IN const wlan_user_scan_cfg * puser_scan_in)
{
    ...
    MOAL_MEMSET(pmadapter, pscan_chan_list, 0x00, buf_size);
    MOAL_MEMSET(pmadapter, pscan_cfg_out, 0x00, sizeof(wlan_scan_cmd_config_tlv));
    ...
    if (keep_previous_scan == MFALSE) {
        MOAL_MEMSET(pmadapter, pmadapter->pscan_table, 0x00,
               sizeof(BSSDescriptor_t) * MRVDRV_MAX_BSSID_LIST);
        pmadapter->num_in_scan_table = 0;
        pmadapter->pbcn_buf_end = pmadapter->bcn_buf;
    }
                     ...
                     IN HostCmd_DS_COMMAND * pcmd, IN t_void * pdata_buf)
{
    ...
    MOAL_MEMCPY(pmpriv->adapter, pscan_cmd->bssid, pscan_cfg->specific_bssid,
           sizeof(pscan_cmd->bssid));
    MOAL_MEMCPY(pmpriv->adapter, pscan_cmd->tlv_buffer, pscan_cfg->tlv_buf,
                     ...
                     IN HostCmd_DS_COMMAND * resp, IN t_void * pioctl_buf)
{
    ...
    for (idx = 0; idx < pscan_rsp->number_of_sets && bytes_left; idx++) {
        ...
        MOAL_MEMSET(pmadapter, bss_new_entry, 0x00, sizeof(BSSDescriptor_t));
        ...
        if (wlan_interpret_bss_desc_with_ie(pmadapter,
            ...
            for (bss_idx = 0; bss_idx < num_in_table; bss_idx++) {
                if (!MOAL_MEMCMP(pmadapter, bss_new_entry->mac_address,
                    ...
                    if ((bss_new_entry->ssid.ssid_len ==
                        ...
                        && (!MOAL_MEMCMP(pmadapter, bss_new_entry->ssid.ssid,
             ...
             */
            if (ptsf_tlv) {
                MOAL_MEMCPY(pmpriv->adapter, &tsf_val,
                       &ptsf_tlv->tsf_data[idx * TSF_DATA_SIZE],
                       sizeof(tsf_val));
                tsf_val = wlan_le64_to_cpu(tsf_val);
                MOAL_MEMCPY(pmpriv->adapter, &bss_new_entry->network_tsf,
            ...
            MOAL_MEMCPY(pmadapter, &pmadapter->pscan_table[bss_idx],
                       ...
                       IN t_u8 * bssid, IN t_u32 mode)
{
    ...
    for (i = 0;
        ...
        if (!wlan_ssid_cmp(pmadapter, &pmadapter->pscan_table[i].ssid, ssid) &&
            (!bssid
             || !MOAL_MEMCMP(pmadapter, pmadapter->pscan_table[i].mac_address, bssid,
                        MLAN_MAC_ADDR_LENGTH))) {
              ...
              IN mlan_802_11_ssid * ssid1, IN mlan_802_11_ssid * ssid2)
{
    ...
    LEAVE();
    return MOAL_MEMCMP(pmadapter, ssid1->ssid, ssid2->ssid, ssid1->ssid_len);
}
                       ...
                       OUT mlan_ssid_bssid * preq_ssid_bssid)
{
    ...
    MOAL_MEMSET(pmadapter, preq_ssid_bssid, 0, sizeof(mlan_ssid_bssid));

    i = wlan_find_best_network_in_list(pmpriv);

    if (i >= 0) {
        preq_bss = &pmadapter->pscan_table[i];
        MOAL_MEMCPY(pmadapter, &preq_ssid_bssid->ssid, &preq_bss->ssid,
               sizeof(mlan_802_11_ssid));
        MOAL_MEMCPY(pmadapter, (t_u8 *) & preq_ssid_bssid->bssid,
                        ...
                        IN t_void * pioctl_buf, IN mlan_802_11_ssid * preq_ssid)
{
    ...
    MOAL_MEMSET(pmpriv->adapter, pscan_cfg, 0x00, sizeof(wlan_user_scan_cfg));

    MOAL_MEMCPY(pmpriv->adapter, pscan_cfg->ssid_list[0].ssid,
...
wlan_save_curr_bcn(IN mlan_private * pmpriv)
{
    ...
    if ((pmpriv->pcurr_bcn_buf == MNULL) ||
        ...
        (MOAL_MEMCMP(pmpriv->adapter, pmpriv->pcurr_bcn_buf, pcurr_bss->pbeacon_buf,
        ...
        if (pmpriv->curr_bcn_size) {
            ...
            if ((ret == MLAN_STATUS_SUCCESS) && pmpriv->pcurr_bcn_buf) {
                MOAL_MEMCPY(pmpriv->adapter, pmpriv->pcurr_bcn_buf,
