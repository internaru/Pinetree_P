...
wlan_enable_aes_key(pmlan_private pmpriv)
{
    ...
    MOAL_MEMSET(pmpriv->adapter, &encrypt_key, 0, sizeof(mlan_ds_encrypt_key));
    encrypt_key.key_len = WPA_AES_KEY_LEN;
    encrypt_key.key_index = MLAN_KEY_INDEX_UNICAST;
    MOAL_MEMCPY(pmpriv->adapter, encrypt_key.key_material,
                       ...
                       IN pmlan_ioctl_req pioctl_req)
{
    ...
    /* SSID */
    MOAL_MEMCPY(pmadapter, &info->param.bss_info.ssid, &pbss_desc->ssid,
           sizeof(mlan_802_11_ssid));

    /* BSSID */
    MOAL_MEMCPY(pmadapter, &info->param.bss_info.bssid, &pbss_desc->mac_address,
    ...
    MOAL_MEMCPY(pmadapter, &info->param.bss_info.capability_info,
    ...
    MOAL_MEMSET(pmadapter, info->param.bss_info.peer_supp_rates, 0,
           sizeof(info->param.bss_info.peer_supp_rates));
    MOAL_MEMCPY(pmadapter, info->param.bss_info.peer_supp_rates,
...
wlan_get_info_ioctl(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    ...
    case MLAN_OID_GET_FW_INFO:
        ...
        MOAL_MEMCPY(pmadapter, &pget_info->param.fw_info.mac_addr, pmpriv->curr_addr,
               MLAN_MAC_ADDR_LENGTH);
                          ...
                          IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_SET) {
        ...
        if ((adhoc_band & BAND_GN)
            ...
            if (!IS_ADHOC_11N(pmadapter->feature_control)) {
                pioctl_req->status_code = MLAN_ERROR_INVALID_PARAMETER;
                LEAVE();
                return MLAN_STATUS_FAILURE;
            }
                           ...
                           IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_GET) {
        ...
        MOAL_MEMCPY(pmadapter, &bss->param.mac_addr, pmpriv->curr_addr,
               MLAN_MAC_ADDR_LENGTH);
        ret = MLAN_STATUS_SUCCESS;
        goto exit;
    }

    MOAL_MEMCPY(pmadapter, pmpriv->curr_addr, &bss->param.mac_addr,
           MLAN_MAC_ADDR_LENGTH);
                                ...
                                IN pmlan_ioctl_req pioctl_req)
{
    ...
    if ((wlan_11d_is_enabled(pmpriv) &&
        ...
        MOAL_MEMSET(pmadapter, &region_chan, 0, sizeof(parsed_region_chan_11d_t));
...
wlan_bss_ioctl_mode(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    ...
    if ((pmpriv->bss_mode != MLAN_BSS_MODE_AUTO)
        && (IS_BSS_MODE(pmpriv->adapter->feature_control))
        ) {
        ret = wlan_prepare_cmd(pmpriv,
...
wlan_query_passphrase(mlan_private * priv, pmlan_ioctl_req pioctl_req)
{
    ...
    MOAL_MEMSET(pmadapter, &sec_pp, 0, sizeof(mlan_ds_passphrase));
    ...
    if (ssid_bssid->ssid.ssid_len == 0) {
        ...
        if (i >= 0) {
            ...
            MOAL_MEMCPY(pmadapter, &sec_pp.ssid, &pbss_desc->ssid,
                   sizeof(mlan_802_11_ssid));
        } else
            MOAL_MEMCPY(pmadapter, &sec_pp.bssid, &ssid_bssid->bssid,
                   MLAN_MAC_ADDR_LENGTH);
    } else {
        MOAL_MEMCPY(pmadapter, &sec_pp.ssid, &ssid_bssid->ssid,
               sizeof(mlan_802_11_ssid));
    }
                           ...
                           IN pmlan_ioctl_req pioctl_req)
{
    ...
    MOAL_MEMSET(pmadapter, buf, 0, MRVDRV_SIZE_OF_CMD_BUFFER);
                               ...
                               IN pmlan_ioctl_req pioctl_req)
{
    ...
    txp_cfg = (HostCmd_DS_TXPWR_CFG *) buf;
    txp_cfg->action = HostCmd_ACT_GEN_SET;
    MOAL_MEMCPY(pmadapter, (t_u8 *) & data,
...
wlan_set_wpa_ie_helper(mlan_private * priv, t_u8 * ie_data_ptr, t_u16 ie_len)
{
    ...
    if (ie_len) {
        ...
        MOAL_MEMCPY(priv->adapter, priv->wpa_ie, ie_data_ptr, ie_len);
        ...
        MOAL_MEMSET(priv->adapter, priv->wpa_ie, 0, sizeof(priv->wpa_ie));
        priv->wpa_ie_len = 0;
...
wlan_set_wapi_ie(mlan_private * priv, t_u8 * ie_data_ptr, t_u16 ie_len)
{
    ...
    if (ie_len) {
        ...
        MOAL_MEMCPY(priv->adapter, priv->wapi_ie, ie_data_ptr, ie_len);
        ...
        MOAL_MEMSET(priv->adapter, priv->wapi_ie, 0, sizeof(priv->wapi_ie));
        priv->wapi_ie_len = (t_u8) ie_len;
                           ...
                           IN pmlan_ioctl_req pioctl_req)
{
    ...
    if ((sec->param.encrypt_key.key_disable == MTRUE) ||
        ...
        if (sec->param.encrypt_key.key_remove == MTRUE) {
            MOAL_MEMSET(pmadapter, &pmpriv->wep_key[index], 0,
        ...
        if (sec->param.encrypt_key.key_len) {
            ...
            /* Cleanup */
            MOAL_MEMSET(pmadapter, pwep_key, 0, sizeof(mrvl_wep_key_t));
            /* Copy the key in the driver */

            MOAL_MEMCPY(pmadapter, pwep_key->key_material,
                           ...
                           IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (sec->param.encrypt_key.key_len == WPA_AES_KEY_LEN) {
        ...
        MOAL_MEMSET(pmpriv->adapter, pmpriv->aes_key.key_param_set.key, 0,
               sizeof(pmpriv->aes_key.key_param_set.key));
        pmpriv->aes_key.key_param_set.key_len = sec->param.encrypt_key.key_len;
        MOAL_MEMCPY(pmpriv->adapter, pmpriv->aes_key.key_param_set.key,
               sec->param.encrypt_key.key_material,
    ...
    if (pmpriv->bss_mode == MLAN_BSS_MODE_IBSS &&
        ...
        MOAL_MEMSET(pmadapter, zero_key_material, 0, sizeof(zero_key_material));
        if (MOAL_MEMCMP
    ...
    if ((sec->param.encrypt_key.key_index == MLAN_KEY_INDEX_UNICAST) &&
        ...
        if (pmpriv->adhoc_aes_enabled == MTRUE &&
            ...
            HEXDUMP("Get ADHOCAES Key", pmpriv->aes_key.key_param_set.key,
                    WPA_AES_KEY_LEN);
            MOAL_MEMCPY(pmadapter, sec->param.encrypt_key.key_material,
    ...
    if (pioctl_req->action == MLAN_ACT_SET) {
        ...
        if (sec->param.passphrase.psk_type == MLAN_PSK_QUERY) {
            if (sec->param.passphrase.ssid.ssid_len == 0) {
                ...
                if (i >= 0) {
                    pbss_desc = &pmadapter->pscan_table[i];
                    MOAL_MEMCPY(pmadapter, &sec->param.passphrase.ssid,
                           &pbss_desc->ssid, sizeof(mlan_802_11_ssid));
                    MOAL_MEMSET(pmadapter, &sec->param.passphrase.bssid, 0,
                           MLAN_MAC_ADDR_LENGTH);
...
wlan_set_gen_ie_helper(mlan_private * priv, t_u8 * ie_data_ptr, t_u16 ie_len)
{
    ...
    if (!ie_len) {
        ...
        if (((pvendor_ie->element_id == WPA_IE)
             ...
             */
            if (pvendor_ie->len > 4) {
                MOAL_MEMCPY(priv->adapter, (t_u8 *) & priv->wps.wps_ie, ie_data_ptr,
                       ie_len);
                HEXDUMP("wps_ie", (t_u8 *) & priv->wps.wps_ie,
                        priv->wps.wps_ie.vend_hdr.len + 2);
            } else {
                /* Only wps oui exist, reset driver wps buffer */
                MOAL_MEMSET(priv->adapter, (t_u8 *) & priv->wps.wps_ie, 0x00,
            ...
            if (ie_len < (sizeof(priv->gen_ie_buf) - priv->gen_ie_buf_len)) {
                ...
                MOAL_MEMCPY(priv->adapter, priv->gen_ie_buf + priv->gen_ie_buf_len,
                       ie_data_ptr, ie_len);
                       ...
                       IN pmlan_ioctl_req pioctl_req)
{
    ...
    case MLAN_IE_TYPE_GEN_IE:
        if (pioctl_req->action == MLAN_ACT_GET) {
            ...
            MOAL_MEMCPY(pmadapter, misc->param.gen_ie.ie_data, pmpriv->wpa_ie,
    ...
    case MLAN_IE_TYPE_ARP_FILTER:
        MOAL_MEMSET(pmadapter, pmadapter->arp_filter, 0,
        ...
        if (misc->param.gen_ie.len > ARP_FILTER_MAX_BUF_SIZE) {
            ...
            MOAL_MEMCPY(pmadapter, pmadapter->arp_filter, misc->param.gen_ie.ie_data,
                   misc->param.gen_ie.len);
                       ...
                       IN pmlan_ioctl_req pioctl_req, IN t_u32 ipv4_addr)
{
    ...
    MOAL_MEMSET(pmadapter, pmadapter->arp_filter, 0, sizeof(pmadapter->arp_filter));
    if (len > ARP_FILTER_MAX_BUF_SIZE) {
        ...
        MOAL_MEMCPY(pmadapter, pmadapter->arp_filter, buf, len);
                          ...
                          IN pmlan_ioctl_req pioctl_req, IN t_u32 ipv4_addr)
{
    ...
    MOAL_MEMSET(pmpriv->adapter, hostcmd, 0, sizeof(mlan_ds_misc_cmd));
    buf = hostcmd->cmd;
    ...
    if (!ipv4_addr) {
        ...
        buf_len += sizeof(HostCmd_DS_MEF_CFG);
        filter = buf + buf_len;
        MOAL_MEMCPY(pmpriv->adapter, filter, fltr_buf, sizeof(fltr_buf));
        MOAL_MEMCPY(pmpriv->adapter, &filter[FLTR_BUF_IP_OFFSET], &ipv4_addr,
               sizeof(ipv4_addr));
                        ...
                        IN pmlan_ioctl_req pioctl_req)
{
    ...
    case MEF_CFG_HOSTCMD:
        ...
        filter = buf + buf_len;
        MOAL_MEMCPY(pmpriv->adapter, filter, mef_cfg->param.cmd_buf.cmd,
                           ...
                           IN pmlan_ioctl_req pioctl_req)
{
    ...
    /* GET operation */
    if (pioctl_req->action == MLAN_ACT_GET) {
        MOAL_MEMCPY(pmadapter, misc->param.ipaddr_cfg.ip_addr,
               pmpriv->ip_addr, IPADDR_LEN);
        misc->param.ipaddr_cfg.op_code = pmpriv->op_code;
        goto done;
    }
    /* only one IP is supported in current firmware */
    MOAL_MEMCPY(pmadapter, &ipv4_addr, misc->param.ipaddr_cfg.ip_addr[0],
           sizeof(t_u32));
    ...
    if (pioctl_req->action == MLAN_ACT_SET) {
        ...
        MOAL_MEMCPY(pmadapter, pmpriv->ip_addr,
    ...
    if (MOAL_MEMCMP
        ...
        if (bss->param.ssid_bssid.ssid.ssid_len)        /* ssid & bssid */
            ...
            i = wlan_find_bssid_in_list(pmpriv,
                                        ...
                                        pmpriv->bss_mode);
        if (i < 0) {
            MOAL_MEMCPY(pmadapter, mac, &bss->param.ssid_bssid.bssid, sizeof(mac));
        ...
        pbss_desc = &pmadapter->pscan_table[i];
        MOAL_MEMCPY(pmadapter, &bss->param.ssid_bssid.ssid, &pbss_desc->ssid,
               sizeof(mlan_802_11_ssid));
        ...
        pbss_desc = &pmadapter->pscan_table[i];
        MOAL_MEMCPY(pmadapter, (t_u8 *) & bss->param.ssid_bssid.bssid,
