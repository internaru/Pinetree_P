                              ...
                              IN custom_ie * ie_data, IN t_u16 idx)
{
    ...
    ENTER();
    MOAL_MEMSET(pmpriv->adapter, del_ie, 0, MAX_IE_SIZE);
    MOAL_MEMCPY(pmpriv->adapter, del_ie, ie_data->ie_buffer, ie_data->ie_length);
    ...
    for (index = 0; index < pmadapter->max_mgmt_ie_index; index++) {
        ...
        cnt = 0;
        while (tmp_len) {
            if (!MOAL_MEMCMP(pmpriv->adapter, tmp_ie, del_ie, del_len)) {
                MOAL_MEMCPY(pmpriv->adapter, ie, pmpriv->mgmt_ie[index].ie_buffer,
                       cnt);
                if (pmpriv->mgmt_ie[index].ie_length > (cnt + del_len))
                    MOAL_MEMCPY(pmpriv->adapter, &ie[cnt],
                           &pmpriv->mgmt_ie[index].ie_buffer[cnt + del_len],
                           (pmpriv->mgmt_ie[index].ie_length -
                            (cnt + del_len)));
                MOAL_MEMSET(pmpriv->adapter, &pmpriv->mgmt_ie[index].ie_buffer, 0,
                       sizeof(pmpriv->mgmt_ie[index].ie_buffer));
                MOAL_MEMCPY(pmpriv->adapter, &pmpriv->mgmt_ie[index].ie_buffer, ie,
                         ...
                         IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_SET) {
        ...
        MOAL_MEMCPY(pmadapter, pmadapter->dbg.last_cmd_id,
               info->param.debug_info.last_cmd_id,
               sizeof(pmadapter->dbg.last_cmd_id));
        MOAL_MEMCPY(pmadapter, pmadapter->dbg.last_cmd_act,
        ...
        MOAL_MEMCPY(pmadapter, pmadapter->dbg.last_cmd_resp_id,
        ...
        MOAL_MEMCPY(pmadapter, pmadapter->dbg.last_event,
        ...
        MOAL_MEMCPY(pmadapter, info->param.debug_info.last_cmd_id,
               pmadapter->dbg.last_cmd_id, sizeof(pmadapter->dbg.last_cmd_id));
        MOAL_MEMCPY(pmadapter, info->param.debug_info.last_cmd_act,
               pmadapter->dbg.last_cmd_act,
               sizeof(pmadapter->dbg.last_cmd_act));
        info->param.debug_info.last_cmd_index = pmadapter->dbg.last_cmd_index;
        MOAL_MEMCPY(pmadapter, info->param.debug_info.last_cmd_resp_id,
               pmadapter->dbg.last_cmd_resp_id,
               sizeof(pmadapter->dbg.last_cmd_resp_id));
        info->param.debug_info.last_cmd_resp_index =
            pmadapter->dbg.last_cmd_resp_index;
        MOAL_MEMCPY(pmadapter, info->param.debug_info.last_event,
                       ...
                       t_u32 head_room, t_u32 malloc_flag)
{
    ...
    if (malloc_flag == MTRUE) {
        ...
        MOAL_MEMSET(pmadapter, pmbuf, 0, sizeof(mlan_buffer));
                        ...
                        IN pmlan_ioctl_req pioctl_req)
{
    ...
    mlan_status ret = MLAN_STATUS_SUCCESS;
    ALIGN4 HostCmd_DS_VERSION_EXT dummy;
    pmlan_callbacks pcb = &pmadapter->callbacks;
    pmlan_buffer pmbuf;
    ...
    if (pioctl_req->action == MLAN_ACT_GET) {
        ...
        for (j = 0; mlan_ops[j]; j++) {
            if (mlan_ops[j]->bss_role == GET_BSS_ROLE(pmpriv)) {
                MOAL_MEMCPY(pmadapter, &pmpriv->ops, mlan_ops[j],
        ...
        MOAL_MEMSET(pmadapter, &dummy, 0, sizeof(HostCmd_DS_VERSION_EXT));
    ...
    if ((misc->param.cust_ie.len == 0) ||
        ...
        while (ioctl_len > 0) {
            ie_data = (custom_ie *) (((t_u8 *) & misc->param.cust_ie)
                                     + app_data_len);
            if (ioctl_len < (ie_data->ie_length + MLAN_CUSTOM_IE_HDR_SIZE)) 
            {
                DPRINTF(DBG_OUTPUT|DBG_ERROR, 
                        ("\nwlan_misc_ioctl_custom_ie_list: ERROR ioctl_len 0x%X < (ie_data->ie_length + MLAN_CUSTOM_IE_HDR_SIZE) 0x%X\n\n",
            ...
            if (MLAN_CUSTOM_IE_AUTO_IDX_MASK == index) {        /* Need to be
                ...
                if (mask == MLAN_CUSTOM_IE_DELETE_MASK) {       /* Automatic
                    ...
                    index = ie_data->ie_index;
					MOAL_MEMSET(pmadapter, ie_data, 0,
                    ...
                    for (i = 0; i < pmadapter->max_mgmt_ie_index; i++) {
                        ...
                        if (index & ((t_u16) 1) << i) {
	                        MOAL_MEMCPY(pmadapter, (t_u8 *) ie_data + len, &i,
	                               sizeof(ie_data->ie_index));
	                        len += sizeof(ie_data->ie_index);
	                        MOAL_MEMCPY(pmadapter, (t_u8 *) ie_data + len,
                    ...
                    MOAL_MEMCPY(pmadapter, tmp_ie + pmpriv->mgmt_ie[index].ie_length,
                    ...
                    MOAL_MEMCPY(pmadapter, &ie_data->ie_buffer,
                ...
                if (ie_data->mgmt_subtype_mask == MLAN_CUSTOM_IE_DELETE_MASK &&
                    ...
                    MOAL_MEMSET(pmadapter, ie_data, 0,
                           sizeof(custom_ie) * MAX_MGMT_IE_INDEX_TO_FW);
                    MOAL_MEMCPY(pmadapter, (t_u8 *) ie_data, &pmpriv->mgmt_ie[index],
                           pmpriv->mgmt_ie[index].ie_length +
                           MLAN_CUSTOM_IE_HDR_SIZE);
                } else {
                    /* 
                    ...
                    if (ie_data->mgmt_subtype_mask ==
                        ...
                        ie_data->ie_length = 0;
                    MOAL_MEMSET(pmadapter, &pmpriv->mgmt_ie[index], 0,
                           sizeof(custom_ie));
                    MOAL_MEMCPY(pmadapter, &pmpriv->mgmt_ie[index], ie_data,
                           sizeof(custom_ie));
                }
...
wlan_add_station_entry(mlan_private * priv, t_u8 * mac)
{
    ...
    MOAL_MEMSET(priv->adapter, sta_ptr, 0, sizeof(sta_node));
    MOAL_MEMCPY(priv->adapter, sta_ptr->mac_addr, mac, MLAN_MAC_ADDR_LENGTH);
                               ...
                               IN t_u8 * payload, IN t_u32 payload_len)
{
    ...
    MOAL_MEMCPY(pmadapter, (t_u8 *) (pevent->event_buf + sizeof(pevent->event_id)),
           payload, payload_len);
                               ...
                               IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (ds_rate->param.rate_cfg.is_rate_auto) {
        MOAL_MEMSET(pmadapter, bitmap_rates, 0, sizeof(bitmap_rates));
