                            ...
                            IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_GET) {
        ...
        MOAL_MEMCPY(pmadapter, cfg->param.addba_reject, pmpriv->addba_reject,
               MAX_NUM_TID);
    } else {
                                 ...
                                 IN pmlan_ioctl_req pioctl_req)
{
    ...
    /* Set MCS for 1x1/2x2 */
    MOAL_MEMSET(pmadapter, (t_u8 *) mcs_set, 0xff, rx_mcs_supp);
    /* Clear all the other values */
    MOAL_MEMSET(pmadapter, (t_u8 *) & mcs_set[rx_mcs_supp], 0,
    ...
    cfg = (mlan_ds_11n_cfg *) pioctl_req->pbuf;
    MOAL_MEMCPY(pmadapter, cfg->param.supported_mcs_set, mcs_set, NUM_MCS_FIELD);
                     ...
                     MrvlIETypes_HTCap_t * pht_cap, t_u8 bands)
{
    ...
    rx_mcs_supp = GET_RXMCSSUPP(pmadapter->usr_dev_mcs_support);
    /* Set MCS for 1x1/2x2 */
    MOAL_MEMSET(pmadapter, (t_u8 *) pht_cap->ht_cap.supported_mcs_set, 0xff,
           rx_mcs_supp);
    /* Clear all the other values */
    MOAL_MEMSET(pmadapter, (t_u8 *) & pht_cap->ht_cap.supported_mcs_set[rx_mcs_supp],
                        ...
                        IN BSSDescriptor_t * pbss_desc, OUT t_u8 ** ppbuffer)
{
    ...
    if (pbss_desc->pht_cap) {
        ...
        MOAL_MEMCPY(pmadapter, (t_u8 *) pht_cap + sizeof(MrvlIEtypesHeader_t),
    ...
    if (pbss_desc->pht_info) {
        if (pmpriv->bss_mode == MLAN_BSS_MODE_IBSS) {
            ...
            MOAL_MEMCPY(pmadapter, (t_u8 *) pht_info + sizeof(MrvlIEtypesHeader_t),
        ...
        MOAL_MEMSET(pmadapter, pchan_list, 0,
    ...
    if (pbss_desc->pbss_co_2040) {
        ...
        MOAL_MEMSET(pmadapter, p2040_bss_co, 0, sizeof(MrvlIETypes_2040BSSCo_t));
        p2040_bss_co->header.type = wlan_cpu_to_le16(BSSCO_2040);
        p2040_bss_co->header.len = sizeof(BSSCo2040_t);

        MOAL_MEMCPY(pmadapter, (t_u8 *) p2040_bss_co + sizeof(MrvlIEtypesHeader_t),
    ...
    if (pbss_desc->pext_cap) {
        ...
        MOAL_MEMSET(pmadapter, pext_cap, 0, sizeof(MrvlIETypes_ExtCap_t));
        pext_cap->header.type = wlan_cpu_to_le16(EXT_CAPABILITY);
        pext_cap->header.len = sizeof(ExtCap_t);

        MOAL_MEMCPY(pmadapter, (t_u8 *) pext_cap + sizeof(MrvlIEtypesHeader_t),
...
wlan_11n_get_txbastream_tbl(mlan_private * priv, int tid, t_u8 * ra)
{
    ...
    while (ptx_tbl != (TxBAStreamTbl *) & priv->tx_ba_stream_tbl_ptr) {
        ...
        if ((!MOAL_MEMCMP(pmadapter, ptx_tbl->ra, ra, MLAN_MAC_ADDR_LENGTH))
            && (ptx_tbl->tid == tid)) {
            LEAVE();
            return ptx_tbl;
        }
                               ...
                               t_u8 * ra, int tid, baStatus_e ba_status)
{
    ...
    if (!wlan_11n_get_txbastream_tbl(priv, tid, ra)) {
        ...
        MOAL_MEMCPY(pmadapter, newNode->ra, ra, MLAN_MAC_ADDR_LENGTH);
...
wlan_send_addba(mlan_private * priv, int tid, t_u8 * peer_mac)
{
    ...
    MOAL_MEMCPY(priv->adapter, &add_ba_req.peer_mac_addr, peer_mac,
...
wlan_send_delba(mlan_private * priv, int tid, t_u8 * peer_mac, int initiator)
{
    ...
    MOAL_MEMSET(priv->adapter, &delba, 0, sizeof(delba));
    ...
    MOAL_MEMCPY(priv->adapter, &delba.peer_mac_addr, peer_mac, MLAN_MAC_ADDR_LENGTH);
...
wlan_get_rxreorder_tbl(mlan_private * priv, rx_reorder_tbl * buf)
{
    ...
    while (rxReorderTblPtr != (RxReorderTbl *) & priv->rx_reorder_tbl_ptr) {
        ...
        MOAL_MEMCPY(priv->adapter, ptbl->ta, rxReorderTblPtr->ta,
               MLAN_MAC_ADDR_LENGTH);
...
wlan_get_txbastream_tbl(mlan_private * priv, tx_ba_stream_tbl * buf)
{
    ...
    while (ptxtbl != (TxBAStreamTbl *) & priv->tx_ba_stream_tbl_ptr) {
        ...
        PRINTM(MINFO, "tid=%d\n", ptbl->tid);
        MOAL_MEMCPY(priv->adapter, ptbl->ra, ptxtbl->ra, MLAN_MAC_ADDR_LENGTH);
        ptbl->amsdu = MFALSE;
        ptxtbl = ptxtbl->pnext;
        ptbl++;
        count++;
