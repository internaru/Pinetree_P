...
#define SIZE_OF_DBG_STRUCT 4
    ...
    if ((MOAL_MEMCMP(pmadapter, &prx_pkt->rfc1042_hdr,
                snap_oui_802_h, sizeof(snap_oui_802_h)) == 0) ||
        ((MOAL_MEMCMP(pmadapter, &prx_pkt->rfc1042_hdr,
                 rfc1042_eth_hdr, sizeof(rfc1042_eth_hdr)) == 0) &&
         MOAL_MEMCMP(pmadapter, &prx_pkt->rfc1042_hdr.snap_type,
        ...
        MOAL_MEMCPY(pmadapter, peth_hdr->src_addr, prx_pkt->eth803_hdr.src_addr,
               sizeof(peth_hdr->src_addr));
        MOAL_MEMCPY(pmadapter, peth_hdr->dest_addr, prx_pkt->eth803_hdr.dest_addr,
...
wlan_ops_sta_process_rx_packet(IN t_void * adapter, IN pmlan_buffer pmbuf)
{
    ...
    if (!IS_11N_ENABLED(priv) ||
        MOAL_MEMCMP(priv->adapter, priv->curr_addr, prx_pkt->eth803_hdr.dest_addr,
    ...
    if (queuing_ra_based(priv)) {
        MOAL_MEMCPY(pmadapter, ta, prx_pkt->eth803_hdr.src_addr,
               MLAN_MAC_ADDR_LENGTH);
    } else {
        if ((rx_pkt_type != PKT_TYPE_BAR) && (prx_pd->priority < MAX_NUM_TID))
            priv->rx_seq[prx_pd->priority] = prx_pd->seq_num;
        MOAL_MEMCPY(pmadapter, ta,
