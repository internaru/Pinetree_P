                        ...
                        int pkt_len, int *pad)
{
    ...
    MOAL_MEMCPY(pmadapter, amsdu_buf, data, (MLAN_MAC_ADDR_LENGTH) * 2);
    ...
    *(t_u16 *) (amsdu_buf + amsdu_buf_offset) = mlan_htons(pkt_len +
    ...
    MOAL_MEMCPY(pmadapter, amsdu_buf + amsdu_buf_offset, &snap, LLC_SNAP_LEN);
    amsdu_buf_offset += LLC_SNAP_LEN;

    MOAL_MEMCPY(pmadapter, amsdu_buf + amsdu_buf_offset, data + dt_offset,
           pkt_len - dt_offset);
    *pad =
...
wlan_11n_form_amsdu_txpd(mlan_private * priv, mlan_buffer * mbuf)
{
    ...
    MOAL_MEMSET(pmadapter, ptx_pd, 0, sizeof(TxPD));
...
wlan_11n_deaggregate_pkt(mlan_private * priv, pmlan_buffer pmbuf)
{
    ...
    while (total_pkt_len > 0) {
        ...
        if (MOAL_MEMCMP(pmadapter, &prx_pkt->rfc1042_hdr,
            ...
            MOAL_MEMMOVE(pmadapter, data + LLC_SNAP_LEN, data, (2 *
                                                           MLAN_MAC_ADDR_LENGTH));
            data += LLC_SNAP_LEN;
        ...
        MOAL_MEMCPY(pmadapter, daggr_mbuf->pbuf + daggr_mbuf->data_offset, data,
               pkt_len);
