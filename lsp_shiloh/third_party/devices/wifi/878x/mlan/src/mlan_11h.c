                                 ...
                                 t_u8 * pout_buf, const t_u8 * pin_ie)
{
    ...
    MOAL_MEMCPY(pmadapter, ptmp_buf, &mrvl_ie_hdr, sizeof(mrvl_ie_hdr));
    ...
    MOAL_MEMCPY(pmadapter, ptmp_buf, pin_ie + 2, pin_ie[1]);
...
wlan_11h_set_ibss_dfs_ie(mlan_private * priv, IEEEtypes_IBSS_DFS_t * pdfs)
{
    ...
    MOAL_MEMSET(adapter, pdfs, 0x00, sizeof(IEEEtypes_IBSS_DFS_t));
     ...
     */
    MOAL_MEMSET(adapter, &initial_map, 0x00, sizeof(initial_map));
    initial_map.unmeasured = 1;

    /* Set the DFS Owner and recovery interval fields */
    MOAL_MEMCPY(adapter, pdfs->dfs_owner, priv->curr_addr, sizeof(pdfs->dfs_owner));
                              ...
                              IEEEtypes_SupportedChannels_t * psup_chan)
{
    ...
    ENTER();
    MOAL_MEMSET(priv->adapter, psup_chan, 0x00,
                         ...
                         const t_void * pinfo_buf)
{
    ...
    MOAL_MEMCPY(priv->adapter, &pcmd_ptr->params.tpc_req, pinfo_buf,
                         ...
                         const t_void * pinfo_buf)
{
    ...
    MOAL_MEMCPY(priv->adapter, &pcmd_ptr->params.chan_sw_ann, pch_sw_ann,
                            ...
                            t_u32 channel, wlan_11h_bss_info_t * p11h_bss_info)
{
    ...
    if (sup_chan_len) {
        ...
        *ppbuffer += sizeof(ie_header);
        ...
        MOAL_MEMCPY(priv->adapter, *ppbuffer, &sup_chan_ie, sup_chan_len);
                       ...
                       t_u32 channel, wlan_11h_bss_info_t * p11h_bss_info)
{
    ...
    if (p11h_bss_info) {
         ...
         */
        MOAL_MEMCPY(adapter, dfs_elem.dfs_owner,
...
wlan_11h_init(mlan_adapter * adapter)
{
    ...
    /* Initialize quiet_ie */
    MOAL_MEMSET(adapter, pquiet, 0, sizeof(IEEEtypes_Quiet_t));
                        ...
                        t_u8 channel)
{
    ...
    MOAL_MEMSET(priv->adapter, &meas_req, 0x00, sizeof(meas_req));
    MOAL_MEMSET(priv->adapter, &meas_rpt, 0x00, sizeof(meas_rpt));
     ...
     */
    MOAL_MEMCPY(priv->adapter, meas_req.mac_addr, priv->curr_addr,
...
wlan_11h_cmdresp_process(mlan_private * priv, const HostCmd_DS_COMMAND * resp)
{
    ...
    case HostCmd_CMD_802_11_TPC_ADAPT_REQ:
        ...
        MOAL_MEMCPY(priv->adapter, priv->adapter->curr_cmd->pdata_buf,
                          ...
                          const t_u8 * pelement)
{
    ...
    case POWER_CONSTRAINT:
        ...
        p11h_bss_info->sensed_11h = MTRUE;
        MOAL_MEMCPY(pmadapter, &p11h_bss_info->power_constraint, pelement,
    ...
    case POWER_CAPABILITY:
        ...
        p11h_bss_info->sensed_11h = MTRUE;
        MOAL_MEMCPY(pmadapter, &p11h_bss_info->power_capability, pelement,
    ...
    case TPC_REPORT:
        ...
        p11h_bss_info->sensed_11h = MTRUE;
        MOAL_MEMCPY(pmadapter, &p11h_bss_info->tpc_report, pelement,
    ...
    case CHANNEL_SWITCH_ANN:
        ...
        p11h_bss_info->sensed_11h = MTRUE;
        MOAL_MEMCPY(pmadapter, &p11h_bss_info->chan_switch_ann, pelement,
    ...
    case QUIET:
        ...
        p11h_bss_info->sensed_11h = MTRUE;
        MOAL_MEMCPY(pmadapter, &p11h_bss_info->quiet, pelement,
    ...
    case IBSS_DFS:
        ...
        p11h_bss_info->sensed_11h = MTRUE;
        MOAL_MEMCPY(pmadapter, &p11h_bss_info->ibss_dfs, pelement,
