...
static uint32_t wpa_supplicant_uid = 0;
				     ...
				     size_t len)
{
    ...
    if (os_memcmp(wpa_s->bssid, test_addr, ETH_ALEN) == 0)
    {
#endif
...
#else
        if (wpa_drv_get_bssid(wpa_s, bssid) == 0 &&
            os_memcmp(bssid, test_addr, ETH_ALEN) !=
            0) {
#endif
			dst = bssid;
...
{
    static bool wpa_pwr_mgr_disallow = false;
    //wpa_printf(MSG_DEBUG, "State: %s -> %s",
	//	   wpa_supplicant_state_txt(wpa_s->wpa_state),
	//	   wpa_supplicant_state_txt(state));
	dbg_printf("WPA State: %s -> %s\n",
		   wpa_supplicant_state_txt(wpa_s->wpa_state),
		   wpa_supplicant_state_txt(state));
    ...
    {
        // Wake the IP stack so it can see we have link status ASAP
        //wake_ip_stack();
        //wifi_intf_set_link_status(WIFI_INTF_LINK_EVENT_UP);
        
        wpa_clear_badwpa_passphr_retry_counter();

        if (wpa_pwr_mgr_disallow) {
            wifi_intf_allow_pwr_mgr(&wpa_supplicant_uid);
            wpa_pwr_mgr_disallow = false;
        }
    } 
    else {
        if (!wpa_pwr_mgr_disallow) {
            wifi_intf_disallow_pwr_mgr(&wpa_supplicant_uid);
            wpa_pwr_mgr_disallow = true;
        }        
    }
}
...
wpa_states wpa_supplicant_get_state(void)
{
    if((gWPAGlobalParams != NULL) && (gWPAGlobalParams->ifaces != NULL))
    {
        return gWPAGlobalParams->ifaces->wpa_state;
    }
			      ...
			      struct wpa_ssid *ssid)
{
	...
	if (ret < 0) {
        ...
        if (wifi_intf_get_tkip_workaround_state() == TKIP_WORKAROUND)
            wifi_intf_set_tkip_workaround_state(TKIP_ASSOCIATE_FAIL);
...
{
	return wpa_supplicant_get_state();
}
