...
//Counter used to detect bad WPA/WPA2 passphrase
int g_wifi_sta_badwpa_passphr_retry_counter;

void wpa_clear_badwpa_passphr_retry_counter(void)
{
    g_wifi_sta_badwpa_passphr_retry_counter = 0;
}
...
static void wpa_supplicant_mark_disassoc(struct wpa_supplicant *wpa_s)
{
    ...
    dbg_printf("wpa_supplicant_mark_disassoc: setting WPA supplicant state to WPA_DISCONNECTED\n");

	//Possibly a bad WPA/WPA2 passphrase. Increment retry count
	g_wifi_sta_badwpa_passphr_retry_counter++;
        if (g_wifi_sta_badwpa_passphr_retry_counter == WIFI_STA_BAD_KEY_RETRY_COUNT)//if the count limit has been reached, report error
        {
			dbg_printf("Enter: wconfig Timer function : Retry limit for bad WPA/WPA2 passphrase reached!\n");    
					 ...
					 struct wpa_scan_result *bss)
{
	...
	while ((ssid->proto & WPA_PROTO_RSN) && bss->rsn_ie_len > 0) {
        ...
        // Check for TKIP cipher only
        if (ie.pairwise_cipher == WPA_CIPHER_TKIP)
        {
#if 0
            // Disable N support
            if (!WlanSetInfraBandBG())
                break;
#endif
            if (wifi_intf_get_tkip_workaround_state() == NO_TKIP_WORKAROUND)
                wifi_intf_set_tkip_workaround_state(TKIP_WORKAROUND);
        }
        else
        {
#if 0
            // Enable N support
            if (!WlanSetInfraBandBGN())
                break;
#endif
            // Remove TKIP cipher
            ssid->pairwise_cipher &= ~WPA_CIPHER_TKIP;

            wifi_intf_set_tkip_workaround_state(NO_TKIP_WORKAROUND);
        }
...
static void wpa_supplicant_event_disassoc(struct wpa_supplicant *wpa_s)
{
    ...
    if (os_memcmp(bssid, test_addr, ETH_ALEN) == 0)
#endif
					 ...
					 union wpa_event_data *data)
{
	...
	if (wpa_s->last_michael_mic_error &&
        ...
        do
        {
            _tx_thread_sleep(1); // Keep waiting until the report is sent
        } while (WirelessLowerLayerHasPacketOutstanding(0));
...
{
    ASSERT(wpa_s != NULL);
    wpa_printf(MSG_DEBUG, "SUP: new event %d\n", event);
	switch (event) {
	case EVENT_ASSOC:
...
#endif /* CONFIG_PEERKEY */
	default:
		wpa_printf(MSG_INFO, "Unknown event %d\n", event);
		break;
	}
    wpa_printf(MSG_DEBUG, "SUP: processed event %d\n", event);
}
