...
#include "os_macros.h"
#include "os_timers.h"
#ifdef HAVE_USB8786_SUPPORT
#include "USB8786_uapsta.h"
#endif
#include "USB8782_uapsta.h"
#include "cal_data.h"
...
extern void do_gettimeofday(struct timeval *time);
...
const char driver_version[] = 
	"USB878X-%s-" MLAN_RELEASE_VERSION "-";

/** USB8782 Card */
#define CARD_USB8782    "USB8782"
/** USB8782 Card */
#define CARD_USB8786    "USB8786"
...
/** Semaphore for atomic read/write */
SEMAPHORE AtomicIncDecSem;

/** Semaphore for ioctl commands */
SEMAPHORE MoalIoctlSem;
/**
...
static int woal_open(struct net_device *dev);
static int woal_close(struct net_device *dev);
static int woal_set_mac_address(struct net_device *dev, void *addr);
static void woal_tx_timeout(struct net_device *dev);
static struct net_device_stats *woal_get_stats(struct net_device *dev);
...
wlan_update_drv_tbl(moal_handle * handle, int drv_mode_local)
{
    ...
    if (fw_name) {
        ...
        if (((struct usb_card_rec *) (handle->card))->card_type ==
            CARD_TYPE_USB8786)
            handle->drv_mode.fw_name = DEFAULT_AP_STA_FW_NAME_8786;
        else
        ...
        if (((struct usb_card_rec *) (handle->card))->card_type ==
            CARD_TYPE_USB8786)
            handle->drv_mode.fw_name = DEFAULT_AP_FW_NAME_8786;
        else
            handle->drv_mode.fw_name = DEFAULT_AP_FW_NAME;
#else
        if (((struct usb_card_rec *) (handle->card))->card_type ==
            CARD_TYPE_USB8786)
            handle->drv_mode.fw_name = DEFAULT_FW_NAME_8786;
        else
...
void
woal_destroy_priv(moal_private * priv)
{
    delete_waitqueue(&priv->ioctl_wait_q);
    delete_waitqueue(&priv->cmd_wait_q);
#ifdef CONFIG_PROC_FS
    delete_waitqueue(&priv->proc_wait_q);
#endif
    delete_waitqueue(&priv->w_stats_wait_q);
}
/** 
...
woal_init_sw(moal_handle * handle)
{
    ...
    if (((struct usb_card_rec *) (handle->card))->card_type ==
        ...
        /* Disable WIFIDIRECT */
        if (!org_drv_mode)
            org_drv_mode = drv_mode;
        drv_mode = org_drv_mode & (~DRV_MODE_WIFIDIRECT);
#endif

        /* Update driver version */
        memcpy((char *) driver_version, CARD_USB8786, strlen(CARD_USB8786));
    } else if (((struct usb_card_rec *) (handle->card))->card_type ==
               CARD_TYPE_USB8782) {
#ifdef WIFI_DIRECT_SUPPORT
        if (org_drv_mode)
            drv_mode = org_drv_mode;
#endif

        /* Update driver version */
        memcpy((char *)driver_version, CARD_USB8782, strlen(CARD_USB8782));
    }
    ...
    InitializeTimer(&handle->reassoc_timer,
...
woal_init_fw_dpc(moal_handle * handle)
{
    ...
    {
        PRINTM(MERROR, "WLAN: Firmware not found!!!\n");
        ret = MLAN_STATUS_FAILURE;
        goto done;
    }
...
#else
        ret = wlan_usb_rx_tx_init(handle);
#endif
    }
    if (ret != MLAN_STATUS_SUCCESS)
        goto done;
    ...
    if (handle->hardware_status != HardwareStatusReady) {
        //woal_moal_debug_info(handle, MTRUE);
        ret = MLAN_STATUS_FAILURE;
        goto done;
    }
    ret = MLAN_STATUS_SUCCESS;
 ...
 */
static mlan_status
woal_request_fw_dpc(moal_handle * handle, firmware *firmware)
{
    mlan_status ret = MLAN_STATUS_SUCCESS;
#if 0
...
{
    mlan_status ret = MLAN_STATUS_SUCCESS;
    firmware module_download_firmware;

    ENTER();

    if (handle) 
    {
        handle->firmware = &module_download_firmware;

        // Check for specific WLAN card type
        if (((struct usb_card_rec *) (handle->card))->card_type == CARD_TYPE_USB8782)
        {
            DPRINTF((DBG_SOFT | DBG_OUTPUT), ("USB8782_FW: 0x%X 0x%X\n",USB8782_FW, sizeof(USB8782_FW)));
            module_download_firmware.data = USB8782_FW;
            module_download_firmware.size = sizeof(USB8782_FW);
        }
#ifdef HAVE_USB8786_SUPPORT
        else if (((struct usb_card_rec *) (handle->card))->card_type == CARD_TYPE_USB8786)

        {
            DPRINTF((DBG_SOFT | DBG_OUTPUT), ("USB8786_FW: 0x%X 0x%X\n",USB8786_FW, sizeof(USB8786_FW)));
            module_download_firmware.data = USB8786_FW;
            module_download_firmware.size = sizeof(USB8786_FW);
        }
#endif
        else
        {
            // Unknown device
            DPRINTF((DBG_SOFT | DBG_OUTPUT), ("Unknown or Unsupported USB WLAN Device\n"));
            handle->firmware = NULL;
            ret = MLAN_STATUS_FAILURE;
        }
    
        if (ret == MLAN_STATUS_SUCCESS) {
            woal_request_fw_dpc(handle, handle->firmware);
        }
    }
    else
    {
...
woal_init_fw(moal_handle * handle)
{
    ...
    if (MTRUE && (handle->skip_fw_dnld != MTRUE)
        ...
        PRINTM(MMSG, "WLAN FW download skipped\n");
...
woal_init_sta_dev(struct net_device *dev, moal_private * priv)
{
    ...
    woal_request_get_fw_info(priv, MOAL_CMD_WAIT, NULL);
...
woal_add_interface(moal_handle * handle, t_u8 bss_index, t_u8 bss_type)
{
    ...
    if (bss_type == MLAN_BSS_TYPE_STA) {
        dev->name = WIFI_IFACE_NAME_STATION;
    }
#endif
#ifdef UAP_SUPPORT
    if (bss_type == MLAN_BSS_TYPE_UAP) {
        dev->name = WIFI_IFACE_NAME_UAP;
    }
#endif
#if defined(WIFI_DIRECT_SUPPORT)
    if (bss_type == MLAN_BSS_TYPE_WIFIDIRECT) {
        dev->name = WIFI_IFACE_NAME_WFD;
    }
#endif
...
woal_open(struct net_device *dev)
{
    ...
    {
        DPRINTF((DBG_SOFT | DBG_OUTPUT), ("woal_open:\n"));
        wifi_intf_sta_set_state(WLAN_ASSOCIATING);
        wifi_intf_sta_set_link_status(WIFI_INTF_LINK_EVENT_UP);
        carrier_on = MTRUE;
	}
#endif
    if (carrier_on == MTRUE) {
        ...
        //dprintf("woal_open:\n");
        //netdrvr_wlan_sta_set_link_status(NET_IFACE_LINK_EVENT_DOWN);
        if (netif_carrier_ok(priv->netdev))
 ...
 */
void
woal_mlan_debug_info(mlan_debug_info * info)
{
    int i;
...
woal_tx_timeout(struct net_device *dev)
{
    ...
    if (priv->num_tx_timeout == NUM_TX_TIMEOUT_THRESHOLD) {
        if (!woal_get_debug_info(priv, MOAL_CMD_WAIT, &info)) {
            woal_mlan_debug_info(&info);
        }
        woal_moal_debug_info(priv->phandle, MFALSE);
    }
#endif
...
woal_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    new_skb = dev_alloc_skb(MLAN_MIN_DATA_HEADER_LEN +
                            sizeof(mlan_buffer) +
                            skb->len);
    MASSERT(new_skb != NULL);

    new_skb->len = skb->len;
    new_skb->txdata = skb->data;
    new_skb->handle = skb->handle;
    new_skb->data = new_skb->data + MLAN_MIN_DATA_HEADER_LEN + sizeof(mlan_buffer);
    
    memcpy(new_skb->data, skb->data, skb->len);

    // The skb from the wireless driver routine netdrvr_wlan_start_xmit is not allocated
    // so the kfree_skb is not required.
    // kfree_skb(skb);
    skb = new_skb;
...
woal_reset_intf(moal_private * priv, t_u8 wait_option, int all_intf)
{
    ...
    if (!handle->reassoc_on && handle->is_reassoc_timer_set) {
        CancelTimer(&handle->reassoc_timer);
        handle->is_reassoc_timer_set = MFALSE;
    }
#endif /* REASSOCIATION */
...
woal_broadcast_event(moal_private * priv, t_u8 * payload, t_u32 len)
{
    ...
    // Pass this data along to the mlanevent.c file so we can send it along
    // to anyone who is interested in these events.
    void process_mlan_drvr_event(char *, t_u8 *, t_u32);
    process_mlan_drvr_event(priv->netdev->name, payload, len);
...
woal_reassociation_thread(void *data)
{
    ...
    moal_private *priv = NULL;
    moal_handle *handle = (moal_handle *) thread->priv;
    wait_queue_t wait = 0;
    int i;
    ...
    for (;;) {
        ...
        if (reassoc_timer_req == MTRUE) {
            ...
            ModTimer(&handle->reassoc_timer, MOAL_TIMER_10S);
        }
    }
 ...
 */
void
woal_moal_debug_info(moal_handle * phandle, u8 flag)
{
...
woal_add_card(void *card)
{
    ...
    PRINTM(ERROR, "woal_add_card!\n");
    DPRINTF((DBG_SOFT | DBG_OUTPUT), ("woal_add_card!\n"));
    ...
    DPRINTF((DBG_SOFT | DBG_OUTPUT), ("woal_add_card m_handle[%d] 0x%X\n", index, m_handle[index]));
    ...
    DPRINTF((DBG_SOFT | DBG_OUTPUT), ("init sw\n"));
    /* Init SW */
    ...
    DPRINTF((DBG_SOFT | DBG_OUTPUT), ("sw init complete\n"));

#ifdef REASSOCIATION
    PRINTM(MINFO, "Starting re-association thread...\n");
    DPRINTF((DBG_SOFT | DBG_OUTPUT), ("start reassociation thread\n"));
    handle->reassoc_thread.priv = (void *) handle;

    wlan_create_thread(woal_reassociation_thread, &handle->reassoc_thread, 
                       "woal_reassoc_service");
#endif /* REASSOCIATION */

    DPRINTF((DBG_SOFT | DBG_OUTPUT), ("woal_init_fw\n"));

    /* Init FW and HW */
    if (MLAN_STATUS_SUCCESS != woal_init_fw(handle)) {
        DPRINTF((DBG_SOFT | DBG_OUTPUT), ("woal_init_fw failed\n"));
        PRINTM(MFATAL, "Firmware Init Failed\n");
        goto err_init_fw;
    }
    DPRINTF((DBG_SOFT | DBG_OUTPUT), ("woal_init_fw done\n"));
    LEAVE();
    return handle;
...
woal_remove_card(void *card)
{
    ...
    priv = handle->priv[0];
    DPRINTF((DBG_SOFT | DBG_OUTPUT), ("woal_remove_card:\n"));
    ...
    delete_waitqueue(&handle->init_wait_q);
    delete_waitqueue(&handle->suspend_wait_q);
    delete_waitqueue(&handle->hs_activate_wait_q);
    delete_waitqueue(&handle->meas_wait_q);
    ...
    FreeTimer(&handle->reassoc_timer);
    wake_up_interruptible(&handle->reassoc_thread.wait_q);
    wlan_terminate_thread(&handle->reassoc_thread);

    MOAL_FREE_SEMAPHORE(&handle->reassoc_sem);
#endif /* REASSOCIATION */

#ifdef STA_SUPPORT
    MOAL_FREE_SEMAPHORE(&handle->async_sem);
#endif
...
woal_init_module(void)
{
    ...
    MOAL_INIT_SEMAPHORE(&AtomicIncDecSem);
    MOAL_INIT_SEMAPHORE(&MoalIoctlSem);
    /* Register with bus */
...
woal_cleanup_module(void)
{
  ...
  exit:
    MOAL_REL_SEMAPHORE(&AddRemoveCardSem);
  exit_sem_err:
    MOAL_FREE_SEMAPHORE(&AddRemoveCardSem);
    MOAL_FREE_SEMAPHORE(&AtomicIncDecSem);
    MOAL_FREE_SEMAPHORE(&MoalIoctlSem);
  /* Unregister from bus */
//    woal_bus_unregister();
    LEAVE();
}
