...
{
    PWLAN_DRV_TIMER timer = NULL;

    if (!
        (timer =
         (PWLAN_DRV_TIMER) kmalloc(sizeof(WLAN_DRV_TIMER), GFP_KERNEL))) 
    {
        return MLAN_STATUS_FAILURE;
    }

    InitializeTimer(timer, callback, pcontext);
...
{
    PWLAN_DRV_TIMER timer = (PWLAN_DRV_TIMER) ptimer;
                 ...
                 IN t_void * ptimer, IN t_u8 periodic, IN t_u32 msec)
{
    if (!ptimer)
        return MLAN_STATUS_FAILURE;
    if (periodic)
        SetPeriodicTimer(ptimer, msec);
    else
        SetTimer(ptimer, msec);
...
moal_stop_timer(IN t_void * pmoal_handle, IN t_void * ptimer)
{
    ...
    CancelTimer((PWLAN_DRV_TIMER) ptimer);
...
{
#if 0 // We use a .h file to load the FW from
    moal_handle *handle = (moal_handle *) pmoal_handle;
                          ...
                          IN pmlan_buffer pmbuf, IN mlan_status status)
{
    ...
    if (pmbuf) {
        ...
        if (skb)
        {
            // If head is NULL this packet must have been from the OS
            // Notify the OS the packet was sent
            if (skb->txdata != NULL) 
                WlanShimDTxPktSent((char *) skb->txdata, skb->len, skb->handle);
                   ...
                   IN pmlan_buffer pmbuf, IN t_u32 port, IN mlan_status status)
{
    ...
    if (pmbuf) {
        ...
        if (port == MLAN_USB_EP_CMD_EVENT)
            wlan_submit_rx_urb(handle, port);
        else
        {
                      ...
                      IN pmlan_buffer pmbuf, IN t_u32 port)
{
    ...
    ret =
        wlan_write_data_async((moal_handle *) pmoal_handle, pmbuf, (t_u8) port);
    LEAVE();
    return ret;
}
...
{
    return wlan_write_data_sync((moal_handle *) pmoal_handle, pmbuf, port,
                                timeout);
}
...
{
    return wlan_read_data_sync((moal_handle *) pmoal_handle, pmbuf, port,
                               timeout);
}
...
moal_recv_packet(IN t_void * pmoal_handle, IN pmlan_buffer pmbuf)
{
    ...
    if (pmbuf) {
        ...
        if (priv) {
            ...
            skb->handle = (void *) pmbuf->bss_index;
            netif_rx(skb);
        }
    }
  done:
...
moal_recv_event(IN t_void * pmoal_handle, IN pmlan_event pmevent)
{
    ...
    case MLAN_EVENT_ID_FW_ADHOC_LINK_SENSED:
        ...
        DPRINTF((DBG_SOFT | DBG_OUTPUT), ("moal_recv_event:\n"));
    ...
    case MLAN_EVENT_ID_DRV_CONNECTED:
        ...
        DPRINTF((DBG_SOFT | DBG_OUTPUT), ("moal_recv_event: link up?\n"));
        // wifi_intf_sta_set_link_status(WIFI_INTF_LINK_EVENT_UP);
        if (!netif_carrier_ok(priv->netdev))
            netif_carrier_on(priv->netdev);
        if (netif_queue_stopped(priv->netdev))
    ...
    case MLAN_EVENT_ID_FW_DISCONNECTED:
        ...
        DPRINTF((DBG_SOFT | DBG_OUTPUT), ("moal_recv_event: link down?\n"));
        ...
        if (priv->phandle->reassoc_on == MTRUE) {
            ...
            ModTimer(&priv->phandle->reassoc_timer, REASSOC_TIMER_DEFAULT);
        } else {
    ...
    case MLAN_EVENT_ID_DRV_REPORT_STRING:
        ...
        woal_send_iwevcustom_event(priv, (t_s8 *)pmevent->event_buf);
        break;
