...
static mlan_ioctl_req *ioctl_req = NULL;
...
woal_copy_mcast_addr(mlan_multicast_list * mlist, struct net_device *dev)
{
    ...
    for (i = 0; i < dev->mc_count; i++) {
        memcpy(&mlist->mac_list[i], dev->mc_list[i], ETH_ALEN);
    }
    LEAVE();
    return i;
}
...
{
    DPRINTF((DBG_LOUD | DBG_OUTPUT), ("woal_request_close:\n"));
...
woal_request_ioctl(moal_private * priv, mlan_ioctl_req * req, t_u8 wait_option)
{
    ...
    tx_semaphore_get(&MoalIoctlSem, TX_WAIT_FOREVER);
    ...
    if ((status != MLAN_STATUS_SUCCESS))
    {
        DPRINTF((DBG_SOFT | DBG_OUTPUT), 
                ("\n-woal_request_ioctl: status=0x%X wait=0x%X req->id=0x%X req=0x%X\n",
                status,
                wait_option,
                req->req_id,
                req));
                  ...
                  t_u8 wait_option, mlan_802_11_ssid * req_ssid)
{
    ...
    // The scan IOCTL request buffer is allocated only once and saved.  
    // Scan sends multiple firmware commands with the same request buffer.
    if (ioctl_req == NULL)
    {
        /* Allocate an IOCTL request buffer */
    ...
    {
        ioctl_req->pbuf = (t_u8 *) ioctl_req + sizeof(mlan_ioctl_req);
        ioctl_req->buf_len = (t_u32) sizeof(mlan_ds_scan);
        ioctl_req->reserved_1 = (t_u32) ((t_u8 *) ioctl_req + sizeof(mlan_ioctl_req) + sizeof(mlan_ds_scan));
    }
