...
#define WPAS_CTRL_PATH "/var/run/wpa_supplicant"
...
#define EAP_FRAG_THRESHOLD_PF    (100)
    ...
    /** WPS state*/
    u8 wps_device_state;
    /** Flag for WPS APs that do not advertise*/
    u8 wps_non_advertise;
    /** Flag for valid WPS data*/
    u8 wps_valid_data;
...
/** Max EAPOL-Start retries(used for SMPBC)*/
#define WPSE_MAX_EAPOL_START 3
...
/** Config Method: Physical Push Button */
#define CONFIG_METHOD_PHYSICAL_PBC        0x0480
/** Config Method: SMPBC */
...
/** RF: 5.0G */
#define RF_50_G                           0x02
/** RF: Dual Band (2.4G + 5.0G) */
#define RF_24_50_G                        0x03
...
/** WPS result file location **/
#define WPS_RESULT_FILE 			"/tmp/wps/wps_status"
#define MAX_WPS_RESULT_FILE_SIZE    5000
/** WPS error file location **/
#define WPS_ERROR_FILE 				"/tmp/wps/wps_error"
#define MAX_WPS_ERROR_FILE_SIZE     50000

//extern void wps_config_skipspace(FILE * fp);
extern int wps_update_device_info(PWPS_INFO pwps_info, WPS_DATA * wps_s, int role);
...
extern int wps_probe_response_resp_type_parser(u8 * message, size_t size,
                                               u8 * resp_type);
extern int wps_registrar_input_PIN(PWPS_INFO pwps_info, char *input);
...
extern void wps_print_uap_role_menu(PWPS_INFO pwps_info);
...
extern int wfd_peer_device_selected(int sel_index);
#endif
...
extern int wps_wfd_session_complete_handler(PWPS_INFO pwps_info,
                                            WPS_DATA * wps_s);
void wlan_wfd_generate_psk(PWPS_INFO pwps_info);
void wlan_wfd_get_psk(PWPS_INFO pwps_info, u8 * psk);

extern void wps_generate_nonce_16B(u8 * buf);
extern void wps_set_device_password_id(PWPS_INFO pwps_info, char *pin);
extern int wps_registrar_set_device_password_id(PWPS_INFO pwps_info,
                                                char *input);
extern int wps_sta_probe_request_prepare(u8 * ptr);
 ...
 *  @param ssid         A pointer to the desired SSID (if any)
 *  @return             WPS_STATUS_SUCCESS--success, otherwise--fail
 */
int wps_enrollee_start(PWPS_INFO pwps_info, WPS_DATA * wps_s, char *ssid);
...
extern int wps_non_interactive;
 ...
 */
int wps_parameter_initialization(PWPS_INFO pwps_info, WPS_DATA * wps_s, int role);
#endif
