...
MrvKDF(u8 * Key, u32 Key_size, u32 TotalKeyLen, u8 * OutKey)
{
    ...
    for (i = 1; i < (iterations + 1); i++) {
        ...
        /**
         * WARNING: When merging new drops of WPS, don't use the sprintf 
         *          routines here.  Newer versions of minSprintf do not
         *          correctly handle writing null bytes (0x00) into the
         *          buffer.  Instead, handle those bytes explicitly.
         */
//        minSprintf((char *) Msg,
...
//                TotalKeyLen % 0x100);
        ...
        Msg[2] = 0x00;
        Msg[3] = (u8)i;
        minSprintf((char *) Msg + 4,"Wi-Fi Easy and Secure Key Derivation");
        Msg[40] = 0x00;
        Msg[41] = 0x00;
        Msg[42] = (u8)(TotalKeyLen / 0x100);
        Msg[43] = (u8)(TotalKeyLen % 0x100);
        Msg[44] = 0x00;
