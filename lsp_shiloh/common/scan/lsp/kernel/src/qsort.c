/*
 *
 * https://lwn.net/Articles/84228/
 * 
 * Added to linux kernel scan module for calibration.
 *
 * David Poole 12-Oct-2012
 */
...
static int __init qsort_init_module(void)
{
    /* 
...
static void __exit qsort_cleanup_module(void)
{
}

module_init( qsort_init_module );
module_exit( qsort_cleanup_module );

EXPORT_SYMBOL(qsort);

MODULE_AUTHOR("Original implementation taken form glibc and credited to Douglas* C. Schmidt (schmidt@ics.uci.edu).");
MODULE_DESCRIPTION("qsort implementation for the Linux kernel");
