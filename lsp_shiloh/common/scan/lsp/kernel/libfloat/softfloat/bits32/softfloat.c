...
-------------------------------------------------------------------------------
*/
float32 uint32_to_float32( unsigned int32 a )
{
...
int32 float64_to_int32( float64 a )
{
    ...
    if ( 0 <= shiftCount ) {
        if ( 11 < shiftCount ) {
            ...
            shortShift64Left(
                 aSig0 | 0x00100000, aSig1, shiftCount, &absZ, &zExtra );
...
float64 float64_sqrt( float64 a )
{
 ...
 invalid:
    ...
    shortShift64Left( aSig0 | 0x00100000, aSig1, 11, &aSig0, &aSig1 );
    zSig0 = estimateSqrt32( aExp, aSig0 ) + 2;
    if ( 0 <= (sbits32) zSig0 ) zSig0 = 0xFFFFFFFF;
    if ( aExp & 1 ) shift64Right( aSig0, aSig1, 1, &aSig0, &aSig1 );
    mul32To64( zSig0, zSig0, &term0, &term1 );
    sub64( aSig0, aSig1, term0, term1, &rem0, &rem1 );
    while ( (sbits32) rem0 < 0 ) {
        --zSig0;
        shortShift64Left( 0, zSig0, 1, &term0, &term1 );
        term1 |= 1;
        add64( rem0, rem1, term0, term1, &rem0, &rem1 );
    }
    shortShift64Left( rem0, rem1, 31, &shiftedRem0, &shiftedRem1 );
    zSig1 = estimateDiv64To32( shiftedRem0, shiftedRem1, zSig0 );
    if ( ( zSig1 & 0x3FF ) <= 5 ) {
        if ( zSig1 == 0 ) zSig1 = 1;
        mul32To64( zSig0, zSig1, &term1, &term2 );
        shortShift64Left( term1, term2, 1, &term1, &term2 );
        sub64( rem1, 0, term1, term2, &rem1, &rem2 );
        mul32To64( zSig1, zSig1, &term2, &term3 );
        sub96( rem1, rem2, 0, 0, term2, term3, &rem1, &rem2, &rem3 );
        while ( (sbits32) rem1 < 0 ) {
            --zSig1;
            shortShift96Left( 0, zSig0, zSig1, 1, &term1, &term2, &term3 );
            term3 |= 1;
            add96(
