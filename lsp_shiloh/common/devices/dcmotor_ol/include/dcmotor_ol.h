...
/**
 * Description: Open loop motor driver API.  This interface relies on external 
 * hardware to close the speed control loop and indicate that the correct 
 ...
 */
typedef struct dcmotor_ol_s dcmotor_ol_t;
 ...
 */
uint32_t dcmotor_wait(dcmotor_ol_t *dcmotor, uint32_t timeout_msecs);
...
void set_encoders_of_paper(dcmotor_ol_t * dcmotor, uint32_t papersize); // add by namja
 ...
 const char * dcmotor_name_by_stepper_t( dcmotor_ol_t *obj );
