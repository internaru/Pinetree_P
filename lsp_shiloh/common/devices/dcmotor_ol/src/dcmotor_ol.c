...
int		cnt=0; //add by namja
uint32_t     current_pos=0; //add by namja
int     current_duty=700; //add by namja
uint32_t     current_pps=0; //add by namja
uint32_t     current_per=0; //add by namja
int     pps_error=0; //add by namja
int     before_pps_error=0; //add by namja
int     diff_pps_error=0; //add by namja
int     fpidcontrol=0; //add by namja
uint32_t paper_enc; // add by namja
int count = 0;
...
void dcmotor_interrupt(int32_t interrupt_count, void *context)
{
	...
	int d_result;	
	cnt++;
//	DBG_ERR("(%d)=>cur_pos (%u), cur_per(%u), cur_pps (%u)\n",cnt, get_mtr6pin_encoder_pos(dcmotor->motor_regs), mot_enc_period_read(dcmotor, &edge), block_freq / mot_enc_period_read(dcmotor, &edge));
//	if(cnt ==3)
//	{
//		DBG_ERR("(%d)=>cur_pos (%u), cur_per(%u), cur_pps (%u)\n",cnt, get_mtr6pin_encoder_pos(dcmotor->motor_regs), mot_enc_period_read(dcmotor, &edge), block_freq / mot_enc_period_read(dcmotor, &edge));
//		gpio_write( duplex_sol, 1 );
//	}
//	else if(cnt == 4)
//	{
//		DBG_ERR("(%d)=>cur_pos (%u), cur_per(%u), cur_pps (%u)\n",cnt, get_mtr6pin_encoder_pos(dcmotor->motor_regs), mot_enc_period_read(dcmotor, &edge), block_freq / mot_enc_period_read(dcmotor, &edge));
//		gpio_write( duplex_sol, 0 );
//	}

	if(fpidcontrol == 0)
	{
		if((cnt % (2)) == 0) // 1control per 50ms
		{
			
			current_pos = get_mtr6pin_encoder_pos(dcmotor->motor_regs);
			current_per = mot_enc_period_read(dcmotor, &edge);
			current_pps = block_freq / current_per;
//			DBG_ERR("(%d)=>cur_pos (%u), cur_per(%u), cur_pps (%u)\n",cnt, current_pos, current_per, current_pps);
//			DBG_ERR("(%d)=>cur_pos (%u), cur_per(%u), cur_pps (%u)\n",cnt, get_mtr6pin_encoder_pos(dcmotor->motor_regs), mot_enc_period_read(dcmotor, &edge), block_freq / mot_enc_period_read(dcmotor, &edge));
			
			if(current_pps > 1950) //1950 = targetpps(2167) * 90%
			{		
				pps_error = 2167 - current_pps;
				diff_pps_error = pps_error - before_pps_error;
				before_pps_error = pps_error;
//				p_result = pps_error *1/1000;		//Initialize value (P_GAIN_MUL/P_GAIN_DIV) = 0.001
				p_result = (int)(pps_error >> 10);		//Initialize value (P_GAIN_MUL/P_GAIN_DIV) = 0.001
				d_result = diff_pps_error * 1/100;	//Initialize value (P_GAIN_MUL/P_GAIN_DIV) = 0.01	
				current_duty = current_duty - p_result - d_result;
//				DBG_ERR("pps_error(%d), before_pps_error(%d), diff_pps_error(%d), p_result(%d), d_result(%d), current_duty(%d)\n", pps_error, before_pps_error,diff_pps_error,p_result,d_result,current_duty);
			}
			else
			{ 
                          ...
                          const motor_connect_t *mtr_connects)
{
    ...
    set_mtr6pin_encoderconfig_initialize( motorRegs,
                                          dcmotor->mtr_connects,
                                          /*MTR6PIN_TBSEL_100_US*/ /*MTR6PIN_TBSEL_1_MS*/MTR6PIN_TBSEL_10_MS );
...
void dcmotor_run( dcmotor_ol_t *dcmotor, dcmotor_speed_t *speed )
{
	...
	set_mtr6pin_encoder_pos(dcmotor->motor_regs, 0); //add by namja
	current_pos = 0; //add by namja
	fpidcontrol = 0; //add by namja
	current_duty = 700; //add by namja
...
//	gpio_open( &duplex_sol, GPIO_BANK_G, 25, 
...
void set_encoders_of_paper(dcmotor_ol_t * dcmotor, uint32_t papersize)
{
	...
	if(psize == 1) paper_enc = 1900*2; //LT
	else if(psize == 2) paper_enc = 1200*2; //EXEC
	else if(psize == 3) paper_enc = 1900*2; //A4
	else if(psize == 4) paper_enc = 1200*2; //B5
	else if(psize == 5) paper_enc = 2400*2; //LG
	else if(psize == 6) paper_enc = 1200*2; //A5
	else if(psize == 7) paper_enc = 1900*2; //cover open
	else {}
//	DBG_ERR("psize (%d)\n", psize);
}
