 ...
 * \file mtr6pin.c
 *
 * \brief Interface routines between a DC motor user and the Gemstone DCMOTOR_BASE2
 *        motor block
 *
 * \warning
...
/* Get the current value of the encoder specified by mtr_pin_func */
UINT32 get_mtr6pin_encoder_current_value(volatile MOTOR_REGS * motor_regs, motor_pin_func_t mtr_pin_func)
{
    UINT32 enc_val;
    enc_val = motor_regs->TS_CNT;

    switch(mtr_pin_func)
    {
        case DC_PIN_FUNC_ENCA:
            return DCMOTOR_BASE_TS_CNT_ENCA_MASK_SHIFT(enc_val);
        case DC_PIN_FUNC_ENCB:
            return DCMOTOR_BASE_TS_CNT_ENCB_MASK_SHIFT(enc_val);
        default:
            ASSERT(mtr_pin_func == DC_PIN_FUNC_ENCA || mtr_pin_func == DC_PIN_FUNC_ENCB);
    }
    return 0;
}
