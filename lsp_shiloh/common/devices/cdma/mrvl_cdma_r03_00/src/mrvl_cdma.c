...
#define DBG_PRFX "cdma: "                    // simple string prepend to the front of all strings.
//#define LOGGER_SUBMOD 0x10000000             // submodule/channel is this bit, if set then log.
#define LOGGER_MODULE_MASK DEBUG_LOGGER_MODULE_DEVICES | DEVICES_LOGGER_SUBMOD_CDMA

#define ERR(...) DBG_PRINTF(LOG_ERR, DBG_PRFX __VA_ARGS__)          // LOG_ERR is the level  
#define SLOG_ERR(...) DBG_MEMLOG(LOG_ERR, DBG_PRFX __VA_ARGS__)      
#define NOTICE(...) DBG_MEMLOG(LOG_NOTICE, DBG_PRFX __VA_ARGS__)    
#define SLOG_NOTICE(...) DBG_MEMLOG(LOG_NOTICE, DBG_PRFX __VA_ARGS__)
...
struct cdma_chain_cmds_s
{
    uint32_t num_cmds;  /// number of descriptors
    uint32_t max_cmds; /// max number of descriptors
    uint32_t element_size; /// array size = max_cmds * element_size
    cdma_link_t *dma_chain_v; /// array of decriptors, also linked list.
    uint32_t *dma_mem_v; /// array of src data decriptors point to.
    void *dma_chain_p; /// _p is physical address _v is virtual
    void *dma_mem_p;  /// _v == _p if physical addresses == virtual addresses
    uint32_t *next_mem_v; /// next free memory in src array.
};
...
static int CdmaProcess(uint8_t *Src, uint8_t *Dest, uint32_t Length, uint32_t Timeout, uint32_t base, CDMA_CALLBACK callback)
{
    int number_of_words, AlignBytes, transfer_size, BytesToCopy, transfer_type=0, Queued=0, threshold;
    uint32_t result;
    ...
    switch( CDMA_CFG_FLOWCTRL_MASK_SHIFT( cdma_registers->CFG ))
    {
        ...
        case 3: // CDMA_DEV_TO_DEV
            ...
            break;
        default:
            XASSERT(NULL == &"CdmaProcess: unsupported transfer type", transfer_type);
            transfer_type = 0;
            break;            
    }
    ...
    if (Length > 0)     // if there is more to do, do it here.
    {
        ...
        // start the xfer here.
        cdma_registers->intEn = CDMA_INTEN_CHAINDONEEN_REPLACE_VAL( cdma_registers->intEn, 1 );
        cdma_registers->CFG =  CDMA_CFG_HALT_REPLACE_VAL( cdma_registers->CFG, 0 );     // not halted
...
error_type_t
cdma_chain_transfer( 
    cdma_handle_t *handle, 
    cdma_chain_cmds_t *chain,
    int32_t dest_pid, 
    ...
    void *context)
{
    ...
    int channel = handle->channel;
    cdma_link_t *c = chain->dma_chain_v;
    ASSERT(channel < config->number_of_channels);
    // ASSERT(cdma_channel[channel].open == true);
    // is this channel in use?
    uint32_t flags;
    ...
    cdma_channel[channel].callback = callback; 
    cdma_channel[channel].context = context; 
    cdma_channel[channel].cont.Length = 0;
    // Disable the DMA channel
    cdma_registers->CFG = CDMA_CFG_ENABLE_REPLACE_VAL( cdma_registers->CFG, 0 );

    cdma_registers->SrcAddr = (uint32_t)c->SourceAddress;
    cdma_registers->DestAddr = (uint32_t)c->DestAddress;
    cdma_registers->Control = c->CntrlWord;
    cdma_registers->intEn = CDMA_INTEN_CHAINDONEEN_REPLACE_VAL( cdma_registers->intEn, 1 );
    cdma_registers->LLIR = (uint32_t)c->NextLink;

    if ( dest_pid < 64 )
    { 
        cdma_registers->CFG = 1
            | 1 << 25 // 32 bit
            | 1 << 20 // memory to peripheral
            | dest_pid << 12; // peripheral id 
    }
    else
    {   // no delay no byte swapping no halt mem-mem no pid no fill AND enable.
        cdma_registers->CFG = 1
            | 1 << 25 // 32 bit
            ;
    }
...
cdma_chain_cmds_t * ///< pointer to dma chain array
...
{
    cdma_chain_cmds_t *s = 0;
    uint32_t sz = 0;
    
    s = (cdma_chain_cmds_t *)MEM_MALLOC( sizeof(cdma_chain_cmds_t) );
    ASSERT(s); if (!s) return 0;

    s->max_cmds = max;

    sz = s->max_cmds * sizeof(cdma_link_t);  
    s->dma_chain_v = MEM_MALLOC_UNCACHED( &s->dma_chain_p, sz, 8);

    sz = s->max_cmds * mem_element_words * sizeof(uint32_t);   
    s->dma_mem_v = MEM_MALLOC_UNCACHED( &s->dma_mem_p, sz, 8);

    s->num_cmds = 0;
    s->element_size = mem_element_words * sizeof(uint32_t);
    s->next_mem_v = s->dma_mem_v;
    s->dma_chain_v[0].NextLink = 0;
    return s;
}

cdma_chain_cmds_t* cdma_chain_array_free(cdma_chain_cmds_t *s)
{
    if (s) 
    {
        memFree(s->dma_chain_v);
        memFree(s->dma_mem_v);
        memFree(s);
    }
    return 0;
...
void cdma_chain_add_cmd_copy(
    cdma_chain_cmds_t *s, 
    volatile uint32_t *dest, ///< physical address
    uint32_t *src_array,
    uint32_t num_src_elements
    )
{
    int i;
    ...
    if ( s->num_cmds > 0 )
    {
        // set previous entry's link to point to this new cmd instead of NULL end of list.
        s->dma_chain_v[s->num_cmds - 1].NextLink = 
            s->dma_chain_p  // physical 
            + (s->num_cmds * sizeof(cdma_link_t)); // offset 
    }
    s->dma_chain_v[s->num_cmds].SourceAddress = 
        s->dma_mem_p // non virtual address 
        + ((uint32_t)s->next_mem_v - (uint32_t)s->dma_mem_v); // offset
    s->dma_chain_v[s->num_cmds].DestAddress = (uint8_t*)dest; 
    s->dma_chain_v[s->num_cmds].NextLink = 0;
    s->dma_chain_v[s->num_cmds].CntrlWord = 
        (4 * num_src_elements) << 16 // bytes to transfer.
        | 1 << 13  // dest increment
        | 1 << 12  // src increment
        | 2 << 10  // dest 32 bit
        | 2 << 8   // src 32 bit
    ...
    for (i=0; i < num_src_elements; i++)
    {
        *s->next_mem_v = src_array[i]; // copy to dma able memory

        SLOG_NOTICE( "dma cmd %x\n", src_array[i]);
        s->next_mem_v++;
    }

    if (DBG_WOULD_PRINTF(LOG_NOTICE)) 
    {
        int prev = 0;
        prev = s->num_cmds ? s->num_cmds - 1 : 0;
        SLOG_NOTICE( "dma addr %x src %x dest %x link %x cntrlword %x \n", 
                    &(s->dma_chain_v[s->num_cmds].SourceAddress),
                    s->dma_chain_v[prev].SourceAddress,
                    s->dma_chain_v[prev].DestAddress,
                    s->dma_chain_v[prev].NextLink,
                    s->dma_chain_v[prev].CntrlWord);
    }
    s->num_cmds++;
}
...
static TX_SEMAPHORE chain_test_semaphore;
static cdma_handle_t *chain_test_channel = 0;

static void from_irq_post_semaphore_function_status( void *cb_data, uint32_t isr_pending_bits ) 
{
    tx_semaphore_put(&chain_test_semaphore); 
}

static void cdma_chained_test( void )
{
    dbg_printf("cdma chain test enter\n");
    ...
    dbg_printf("cdma chain test exit fail cnt = %d\n", fail);    
}
...
int cdma_debug_callback(int argc, char *argv[])
{
    ...
    if( argc < 2 ) 
    {
        dbg_printf("Supported commands:\n");
#ifdef HAVE_UNIT_TEST
        dbg_printf("'cdma test' - runs all cdma tests\n");
#endif
        dbg_printf("'cdma chain' - runs cdma chain tests\n");
        return 0;
    }
    ...
    else if((strncmp( argv[pos],"test",5 ) == 0 ) && ( argc == 2 ) ) 
    {
        ...
        if(line) 
        {
            dbg_printf("ERROR in mrvl_cdma.c line %d\n", line);
        }
    }
