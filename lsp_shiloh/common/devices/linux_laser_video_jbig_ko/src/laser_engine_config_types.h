/*
 * ============================================================================
 * Copyright (c) 2010-2013 Marvell Semiconductor, Inc. All Rights Reserved.
 *
 *                         Marvell Confidential
 * ============================================================================
 */

#ifndef _LASER_ENGINE_CONFIG_TYPES_H_
#define _LASER_ENGINE_CONFIG_TYPES_H_
...
typedef struct laser_engine_video_config_s {
    ...
    bool report_video_event[MAX_NUM_VIDEO_CHAN];    // Map which channel should reports video events.
    ...
    beam_direction_t beam_direction[MAX_NUM_VIDEO_CHAN];
    ...
    // throughput.
    COLOR chan_to_print_plane_color[MAX_NUM_VIDEO_CHAN];
    ...
    uint8_t video_chan_hsync_input[MAX_NUM_VIDEO_CHAN]; 
    ...
    // documentation for details.
    uint8_t top_to_print_plane[MAX_NUM_VIDEO_CHAN];
    ...
    uint8_t topq_hsync_sel[MAX_NUM_HSYNC_INPUTS];
    ...
    // channel in use.
    bool active_video_channels[MAX_NUM_VIDEO_CHAN];
    ...
    uint32_t pulse_code_1_bit_array[MAX_NUM_PCODE_REGS];
    uint32_t pulse_code_2_bit_array[MAX_NUM_PCODE_REGS];
    uint32_t pulse_code_4_bit_array[MAX_NUM_PCODE_REGS];

    // And error diffusion pulse codes are here:
    uint32_t ed_pulse_code_1_bit_array[MAX_NUM_PCODE_REGS];
    uint32_t ed_pulse_code_2_bit_array[MAX_NUM_PCODE_REGS];
    uint32_t ed_pulse_code_4_bit_array[MAX_NUM_PCODE_REGS];
    ...
    // in lines.
    uint8_t  top_of_page_skew_in_lines[MAX_NUM_VIDEO_CHAN];
    ...
    uint32_t side_margin_reference_in_sub_pixels[MAX_NUM_VIDEO_CHAN];

    uint32_t pixel_freq_in_hz[MAX_NUM_VIDEO_CHAN];  // Expect these all to be the same, except for tandem color
    ...
    uint32_t num_mirror_facets[MAX_NUM_MIRRORS];   
