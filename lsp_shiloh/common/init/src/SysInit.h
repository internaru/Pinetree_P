...
#include "e-fuse_api.h"
    ...
    #define INITIALIZE_WIFI_SETTER() do \
		                     { \
								 int init_wifisetter( void ); \
								 init_wifisetter();\
							 } \
		                     while(0)
#else
    #define INITIALIZE_WIFI_SETTER() _DO_NOTHING
#endif

#ifdef HAVE_IFAX
	#include "iFaxMailReceiver.h"

    #define INITIALIZE_IFAX() do \
		                     { \
								 int init_iFax( void ); \
								 getIFaxReceiveInformation(); \
								 init_iFax();\
							 } \
		                     while(0)
#else
    #define INITIALIZE_IFAX() _DO_NOTHING
#endif
