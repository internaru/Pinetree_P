...
#ifndef INC_NVRAM_DEV_NAND_H
#define INC_NVRAM_DEV_NAND_H

/**
 *  \file nvram_dev_NAND.h
 *
 *  \brief External interface for NVRAM variable block storage on NAND devices
 *
 *  This shim sits between the NV Manager and the Partition Manager and is 
 *  intended to hide NAND specific knowledge from the upper parts of the 
...
/**
 *  \brief Initialize the NV NAND device shim
 *
 *  Must be called before using any of the other NAND device shim routines.
 *
 **/
...
/**
 *  \brief Return requirements for the NAND based container partition
 *
 *  This routine returns the container partition requirements for the specified
 *  memory device.  NAND container partitions require two or more aligned blocks 
 ...
 *    - verify that it meets needs from GetPartitionNeedsNAND(): a partition
 ...
 **/
error_type_t ReleaseVarBlockNAND(char *label, MEMORY_DEVICE memDev);
