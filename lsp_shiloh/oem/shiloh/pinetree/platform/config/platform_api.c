...
//        plat_set_install_date();
    ...
    DPRINTF(DBG_SOFT|DBG_OUTPUT,("nvram config init status %d\n",status));
    posix_create_thread(&uptime_tid, Uptime, 0, "tUptime", UptimeStack, UPTIME_STACK_SIZE, POSIX_THR_PRI_NORMAL);
}
...
void platvars_shutdown()
{
    ...
    result = pthread_cancel(uptime_tid);
    ASSERT(result == 0);
#ifdef HAVE_NVRAM
    nvram_set_var(plat_nvm_handle, &plat_vars);
#endif
