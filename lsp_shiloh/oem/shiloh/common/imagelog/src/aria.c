...
#define LOG_PATH_LENGTH 600

int ARIA_encode_path(char *in_file, char *out_file, int crypt_flag, int file_option)
{
    char cmd_line[LOG_PATH_LENGTH];
    int aria_result = 0;
    memset(cmd_line, 0x00, LOG_PATH_LENGTH);

    aria_result = ARIA_core(in_file, out_file, crypt_flag);

    if (file_option == 0)
        snprintf(cmd_line, LOG_PATH_LENGTH, "rm %s", in_file);
    system(cmd_line);
...
int ARIA_do_encode(char *in_file, int crypt_flag){
    char out_file[IMAGELOG_FILENAME_LENGTH+8];
    char cmd_line[LOG_PATH_LENGTH];
    int aria_result = 0;
    memset(out_file, 0x00, IMAGELOG_FILENAME_LENGTH+8);
    memset(cmd_line, 0x00, LOG_PATH_LENGTH);
    sprintf(out_file, "%s.encrypt", in_file);
    ...
    memset(cmd_line, 0x00, LOG_PATH_LENGTH);
    snprintf(cmd_line, LOG_PATH_LENGTH, "mv %s %s", out_file, in_file);
    system(cmd_line);

    return aria_result;
}

int ARIA_core(char *in_file, char *out_file, int crypt_flag) // flag == 0 : encrypt, flag == 1: decrypt
{
    int i=0;
    int len=0;
//    int padding_len=0;
    Byte w_buf[FREAD_COUNT];
    Byte r_buf[FREAD_COUNT];
//    Byte rk[16*17], c[16], *b, mk[32];
    ...
    FILE *kfp=fopen("/app/ARIA-key","rb");
    if( kfp == NULL ){
        fprintf(stderr,"[ERROR] %d can not fopen('%s')\n",__LINE__,in_file);
        return -1;
    }
    len = fread(mk, 1, 32, kfp);
    fclose(kfp);
    ...
    if(crypt_flag == 0) {
        //printf("e%d: %d\t", count, len);
        EncKeySetup(mk, rk, 256);
    }
    else if(crypt_flag == 1) {
        //printf("d%d: %d\t", count, len);
        DecKeySetup(mk, rk, 256);
    }
    ...
    while(1){
        count++;
        for (i = 0; i < FREAD_COUNT; i++){
            r_buf[i] = 0x00;
            w_buf[i] = 0x00;
        }
//        memset(r_buf, 0x0000, FREAD_COUNT);
//        memset(w_buf, 0x0000, FREAD_COUNT);
        if (0 < (len = fread( r_buf, RW_SIZE ,FREAD_COUNT, fp))){
            Crypt(r_buf, 16, rk, w_buf);
            if(len < FREAD_COUNT){
                //printf("plaintext : "); printBlock(r_buf); printf("\n");
                //printf("ciphertext: "); printBlock(w_buf); printf("\n");
            }
            fwrite(w_buf, RW_SIZE ,FREAD_COUNT ,wfp);
        }
        else
            break;
    } 
